 ...
 */
class Blob extends \Application\DeskPRO\Entity\Blob implements \Doctrine\ORM\Proxy\Proxy
{
    ...
    {
        if ($this->__isInitialized__ === false) $this->__load();
        return parent::getDownloadUrl($absolute, $use_file_url);
    }

    public function getEmbedCode($for_ticket = false, $type = 'image')
    {
        if ($this->__isInitialized__ === false) $this->__load();
        return parent::getEmbedCode($for_ticket, $type);
    }

    public function getThumbnailUrl($size = 50, $absolute = false)
    {
        if ($this->__isInitialized__ === false) $this->__load();
        return parent::getThumbnailUrl($size, $absolute);
    }

    public function getFilenameSafe()
    {
        if ($this->__isInitialized__ === false) $this->__load();
        return parent::getFilenameSafe();
    }

    public function getNameHash()
    {
        if ($this->__isInitialized__ === false) $this->__load();
        return parent::getNameHash();
    }
    ...
    public function addLabel(\Application\DeskPRO\Entity\LabelBlob $label)
    {
        if ($this->__isInitialized__ === false) $this->__load();
    ...
    {
        if ($this->__isInitialized__ === false) $this->__load();
        return parent::setStorageLocPref($storage_loc);
    }
    ...
    {
        return array('__isInitialized__', 'id', 'sys_name', 'storage_loc', 'storage_loc_pref', 'save_path', 'file_url', 'filename', 'filesize', 'content_type', 'authcode', 'blob_hash', 'is_media_upload', 'title', 'dim_w', 'dim_h', 'date_created', 'is_temp', 'date_cleanup', 'original_blob', 'labels');
    }
