...
/* AdminBundle:Settings:quick-setup-cron-errors.html.twig */
class __TwigTemplate_bf56e69fa5a716bfa5aecb34368740a2 extends \Application\DeskPRO\Twig\Template
{
...
\t";
        ...
        if (isset($context["web_ini_path"])) { $_web_ini_path_ = $context["web_ini_path"]; } else { $_web_ini_path_ = null; }
        if (isset($context["is_zendserver"])) { $_is_zendserver_ = $context["is_zendserver"]; } else { $_is_zendserver_ = null; }
        if (((((!$_ini_path_) && $_web_ini_path_) && $_is_zendserver_) && $this->env->getExtension('deskpro_templating')->getInstanceAbility("is_windows"))) {
            // line 9
            echo "\t\t<div style=\"background-color: #FDFFC2; padding: 5px; margin: 15px;\">
\t\t\t<p>
...
\t\t\t";
            ...
            if ($_web_ini_path_) {
                // line 18
                echo "\t\t\t\t<p>
\t\t\t\t\tThe php.ini file you are currently using on the web server is located at: ";
                // line 19
                if (isset($context["web_ini_path"])) { $_web_ini_path_ = $context["web_ini_path"]; } else { $_web_ini_path_ = null; }
                echo twig_escape_filter($this->env, $_web_ini_path_, "html", null, true);
                echo "<br />
...
\t\t";
        // line 26
        $context["show_phpini"] = false;
        // line 27
        echo "\t";
        if (isset($context["error_codes"])) { $_error_codes_ = $context["error_codes"]; } else { $_error_codes_ = null; }
        $context['_parent'] = (array) $context;
        $context['_seq'] = twig_ensure_traversable($_error_codes_);
...
\t\t\t";
            ...
            if (($_code_ == "php_version")) {
                // line 30
                echo "\t\t\t\tThe PHP version used to execute the scheduled task is outdated. Your web server and your command-line
\t\t\t\tversions of PHP are different. You need to upgrade your command-line version of PHP to at least 5.3.2.
\t\t\t";
            } elseif ((($_code_ == "pdo_ext") || ($_code_ == "pdo_mysql_ext"))) {
                // line 33
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 34
                echo "\t\t\t\tPHP does not have the <a href=\"http://php.net/manual/en/pdo.installation.php\">pdo and pdo_mysql</a> extension enabled.
\t\t\t\tYou will need to activate these extensions in php.ini.
\t\t\t";
            } elseif (($_code_ == "iconv_ext")) {
                // line 37
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 38
                echo "\t\t\t\tPHP does not have the <a href=\"http://php.net/manual/en/iconv.installation.php\">iconv</a> or
...
\t\t\t";
            } elseif (($_code_ == "json_ext")) {
                // line 42
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 43
                echo "\t\t\t\tPHP does not have the <a href=\"http://php.net/manual/en/json.installation.php\">json extension</a> enabled.
\t\t\t\tYou will need to activate this extension in php.ini.
\t\t\t";
            } elseif (($_code_ == "libxml_ext")) {
                // line 46
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 47
                echo "\t\t\t\tPHP does not have the <a href=\"http://php.net/manual/en/book.libxml.php\">libxml extension</a> enabled.
\t\t\t\tYou will need to rebuild PHP with libxml enabled.
\t\t\t";
            } elseif (($_code_ == "libxml_version")) {
                // line 50
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 51
                echo "\t\t\t\tYour version of PHP was built against a very old version of libxml (libxml is used by the <a href=\"http://php.net/manual/en/book.libxml.php\">libxml extension</a>).
...
\t\t\t";
            } elseif (($_code_ == "ctype_ext")) {
                // line 54
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 55
                echo "\t\t\t\tPHP does not have the <a href=\"http://php.net/manual/en/ctype.installation.php\">ctype extension</a> enabled.
\t\t\t\tYou will need to activate this extension in php.ini.
\t\t\t";
            } elseif (($_code_ == "tokenizer_ext")) {
                // line 58
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 59
                echo "\t\t\t\tPHP does not have the <a href=\"http://php.net/manual/en/tokenizer.installation.php\">tokenizer extension</a> enabled.
\t\t\t\tYou will need to activate this extension in php.ini.
\t\t\t";
            } elseif (($_code_ == "memory_limit")) {
                // line 62
                echo "\t\t\t\t";
                $context["show_phpini"] = true;
                // line 63
                echo "\t\t\t\tPHP's <a href=\"http://php.net/manual/en/ini.core.php#ini.memory-limit\">memory limit</a> is set too low.
\t\t\t\tEdit your php.ini and set memory_limit to at least 128M.
\t\t\t";
            } elseif (($_code_ == "php_functions")) {
                // line 66
                echo "\t\t\t\tPHP is disabling functions with the <a href=\"http://php.net/manual/en/ini.core.php#ini.disable-functions\">disable_functions</a> directive.
\t\t\t\tEdit your php.ini and remove the line setting disable_functions.
\t\t\t";
            } elseif (($_code_ == "data_write")) {
                // line 69
                echo "\t\t\t\tThe data directory is not writable by the user that executes the scheduled task.
\t\t\t\tEnsure that the user who runs the task is able to write to your data directory at ";
                // line 70
                if (isset($context["data_dir"])) { $_data_dir_ = $context["data_dir"]; } else { $_data_dir_ = null; }
                echo twig_escape_filter($this->env, $_data_dir_, "html", null, true);
                echo "
\t\t\t";
            } elseif (($_code_ == "db_connect")) {
                // line 72
                echo "\t\t\t\tThe scheduled task was unable to connect to the MySQL database.
\t\t\t";
            } else {
                // line 74
                echo "\t\t\t\tAn unknown error occurred that prevented scheduled tasks from running. Here is the error log file from data/logs/error.log:
\t\t\t\t<textarea style=\"font-size: 11px; font-family: Consolas, Monaco, monospace; width: 90%; height: 150px\">";
                // line 75
                if (isset($context["code"])) { $_code_ = $context["code"]; } else { $_code_ = null; }
                echo twig_escape_filter($this->env, $_code_, "html", null, true);
                echo ": ";
                if (isset($context["error_log"])) { $_error_log_ = $context["error_log"]; } else { $_error_log_ = null; }
                echo twig_escape_filter($this->env, $_error_log_, "html", null, true);
                echo "</textarea>
...
\t";
        ...
        // line 80
        echo "\t";
        if (isset($context["show_phpini"])) { $_show_phpini_ = $context["show_phpini"]; } else { $_show_phpini_ = null; }
        if ($_show_phpini_) {
            // line 81
            echo "\t\t<p>
...
\t\t\t";
            ...
            if ($_ini_path_) {
                // line 85
                echo "\t\t\t\tWe have detected the php.ini file being used by the scheduled task is:<br/><var>";
                if (isset($context["ini_path"])) { $_ini_path_ = $context["ini_path"]; } else { $_ini_path_ = null; }
...
\t\t\t";
                ...
                // line 87
                echo "\t\t\t\tWe were unable to determine the path to the php.ini file used by the scheduled task, you will need to locate it manually.
\t\t\t";
            }
            // line 89
            echo "\t\t</p>
\t";
    ...
    {
        return "AdminBundle:Settings:quick-setup-cron-errors.html.twig";
    }
