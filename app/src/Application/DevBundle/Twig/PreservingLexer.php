 ...
 */
class PreservingLexer implements \Twig_LexerInterface
{
    protected $tokens;
    protected $code;
    protected $cursor;
    ...
    public function __construct(\Twig_Environment $env, array $options = array())
    {
        ...
        $this->regexes = array(
            ...
            'lex_comment'         => '/(.*?'.preg_quote($this->options['whitespace_trim'], '/').'?'.preg_quote($this->options['tag_comment'][1], '/').')/msA',
    ...
    protected function lexData()
    {
        ...
        switch ($this->positions[1][$this->position][0]) {
            case $this->options['tag_comment'][0]:
                $this->pushToken(Token::WHITESPACE_TYPE, $this->positions[0][$this->position][0]);
                $this->lexComment();
                break;
            ...
            case $this->options['tag_block'][0]:
                ...
                if (preg_match($this->regexes['lex_block_raw'], $this->code, $match, null, $this->cursor)) {
                    $this->pushToken(Token::WHITESPACE_TYPE, $this->positions[0][$this->position][0]);
                    $this->pushToken(Token::WHITESPACE_TYPE, $match[0]);
                ...
                // {% line \d+ %}
                    ...
                    $this->pushToken(Token::BLOCK_START_TYPE, $this->positions[0][$this->position][0]);
                    $this->pushState(self::STATE_BLOCK);
                }
                break;

            case $this->options['tag_variable'][0]:
                $this->pushToken(Token::VAR_START_TYPE, $this->positions[0][$this->position][0]);
    ...
    protected function lexBlock()
    {
        ...
        if (empty($this->brackets) && preg_match($this->regexes['lex_block'], $this->code, $match, null, $this->cursor)) {
            $this->pushToken(Token::BLOCK_END_TYPE, $match[1]);
            $this->moveCursor($match[0]);
            $this->popState();
        } else {
            $this->lexExpression();
    ...
    protected function lexVar()
    {
        ...
        if (empty($this->brackets) && preg_match($this->regexes['lex_var'], $this->code, $match, null, $this->cursor)) {
            $this->pushToken(Token::VAR_END_TYPE, $match[1]);
            $this->moveCursor($match[0]);
            $this->popState();
        } else {
            $this->lexExpression();
    ...
    protected function lexRawData()
    {
        ...
        $this->pushToken(Token::TEXT_TYPE, $text);
        $this->pushToken(Token::WHITESPACE_TYPE, $match[0][0]);
    }

    protected function lexComment()
    {

        if (!preg_match($this->regexes['lex_comment'], $this->code, $match, null, $this->cursor)) {
    ...
    protected function lexString()
    {
        if (preg_match($this->regexes['interpolation_start'], $this->code, $match, null, $this->cursor)) {
            ...
            $this->pushToken(Token::INTERPOLATION_START_TYPE, $match[0]);
            $this->moveCursor($match[0]);
