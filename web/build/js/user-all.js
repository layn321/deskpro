window.Modernizr=function(a,b,c){function B(a){j.cssText=a}function C(a,b){return B(n.join(a+";")+(b||""))
...
}(this,this.document);(function(g,b){function k(){var a=e.elements;return"string"==typeof a?a.split(" "):a
}function l(a){var c={},f=a.createElement,b=a.createDocumentFragment,d=b();a.createElement=function(a){if(!e.shivMethods){return f(a)
}var b;b=c[a]?c[a].cloneNode():m.test(a)?(c[a]=f(a)).cloneNode():f(a);return b.canHaveChildren&&!n.test(a)?d.appendChild(b):b
...
d.createElement(a);return'c("'+a+'")'})+");return n}")(e,d)}function h(a){var c;if(a.documentShived){return a
}if(e.shivCSS&&!i){c=a.createElement("p");var b=a.getElementsByTagName("head")[0]||a.documentElement;
...
c=!!b.insertBefore(c.lastChild,b.firstChild)}j||(c=!l(a));if(c){a.documentShived=c}return a}var d=g.html5||{},n=/^<|^(?:button|form|map|select|textarea|object|iframe|option|optgroup)$/i,m=/^<|^(?:a|b|button|code|div|fieldset|form|h1|h2|h3|h4|h5|h6|i|iframe|img|input|label|li|link|ol|option|p|param|q|script|select|span|strong|style|table|tbody|td|textarea|tfoot|th|thead|tr|ul)$/i,i,j;
(function(){var a=b.createElement("a");a.innerHTML="<xyz></xyz>";i="hidden" in a;if(!(a=1==a.childNodes.length)){a:{try{b.createElement("a")
}catch(c){a=!0;break a}a=b.createDocumentFragment();a="undefined"==typeof a.cloneNode||"undefined"==typeof a.createDocumentFragment||"undefined"==typeof a.createElement
}}j=a})();var e={elements:d.elements||"abbr article aside audio bdi canvas data datalist details figcaption figure footer header hgroup mark meter nav output progress section summary time video",shivCSS:!1!==d.shivCSS,shivMethods:!1!==d.shivMethods,type:"default",shivDocument:h};
g.html5=e;h(b)})(this,document);Modernizr.addTest("osmac",function(){if(!navigator||!navigator.appVersion){return false
...
})}})(jQuery);jQuery.cookie=function(key,value,options){if(arguments.length>1&&(value===null||typeof value!=="object")){options=jQuery.extend({},options);
...
}):a.tmpl=b})(this);(function(b){var a=function(c){return parseInt(c,10)||0};b.each(["min","max"],function(d,c){b.fn[c+"Size"]=function(g){var f,e;
if(g){if(g.width!==undefined){this.css(c+"-width",g.width)}if(g.height!==undefined){this.css(c+"-height",g.height)
}return this}else{f=this.css(c+"-width");e=this.css(c+"-height");return{width:(c==="max"&&(f===undefined||f==="none"||a(f)===-1)&&Number.MAX_VALUE)||a(f),height:(c==="max"&&(e===undefined||e==="none"||a(e)===-1)&&Number.MAX_VALUE)||a(e)}
...
}setTimeout(l,10)})}function j(l){$el=a(l);$el.addClass($el.attr("type"));b(l)}function g(l){a(l).addClass("uniform");
b(l)}function i(o){var m=a(o);var p=a("<div>"),l=a("<span>");p.addClass(k.buttonClass);if(k.useID&&m.attr("id")!=""){p.attr("id",k.idPrefix+"-"+m.attr("id"))
}var n;if(m.is("a")||m.is("button")){n=m.text()}else{if(m.is(":submit")||m.is(":reset")||m.is("input[type=button]")){n=m.attr("value")
}}n=n==""?m.is(":reset")?"Reset":"Submit":n;l.html(n);m.css("opacity",0);m.wrap(p);m.wrap(l);p=m.closest("div");
l=m.closest("span");if(m.is(":disabled")){p.addClass(k.disabledClass)}p.bind({"mouseenter.uniform":function(){p.addClass(k.hoverClass)
},"mouseleave.uniform":function(){p.removeClass(k.hoverClass);p.removeClass(k.activeClass)},"mousedown.uniform touchbegin.uniform":function(){p.addClass(k.activeClass)
},"mouseup.uniform touchend.uniform":function(){p.removeClass(k.activeClass)},"click.uniform touchend.uniform":function(r){if(a(r.target).is("span")||a(r.target).is("div")){if(o[0].dispatchEvent){var q=document.createEvent("MouseEvents");
q.initEvent("click",true,true);o[0].dispatchEvent(q)}else{o[0].click()}}}});o.bind({"focus.uniform":function(){p.addClass(k.focusClass)
},"blur.uniform":function(){p.removeClass(k.focusClass)}});a.uniform.noSelect(p);b(o)}function e(o){var m=a(o);
var p=a("<div />"),l=a("<span />");if(!m.css("display")=="none"&&k.autoHide){p.hide()}p.addClass(k.selectClass);
if(k.useID&&o.attr("id")!=""){p.attr("id",k.idPrefix+"-"+o.attr("id"))}var n=o.find(":selected:first");
if(n.length==0){n=o.find("option:first")}l.html(n.html());o.css("opacity",0);o.wrap(p);o.before(l);p=o.parent("div");
l=o.siblings("span");o.bind({"change.uniform":function(){l.text(o.find(":selected").html());p.removeClass(k.activeClass)
},"focus.uniform":function(){p.addClass(k.focusClass)},"blur.uniform":function(){p.removeClass(k.focusClass);
p.removeClass(k.activeClass)},"mousedown.uniform touchbegin.uniform":function(){p.addClass(k.activeClass)
},"mouseup.uniform touchend.uniform":function(){p.removeClass(k.activeClass)},"click.uniform touchend.uniform":function(){p.removeClass(k.activeClass)
},"mouseenter.uniform":function(){p.addClass(k.hoverClass)},"mouseleave.uniform":function(){p.removeClass(k.hoverClass);
p.removeClass(k.activeClass)},"keyup.uniform":function(){l.text(o.find(":selected").html())}});if(a(o).attr("disabled")){p.addClass(k.disabledClass)
}a.uniform.noSelect(l);b(o)}function f(n){var m=a(n);var o=a("<div />"),l=a("<span />");if(!m.css("display")=="none"&&k.autoHide){o.hide()
}o.addClass(k.checkboxClass);if(k.useID&&n.attr("id")!=""){o.attr("id",k.idPrefix+"-"+n.attr("id"))}a(n).wrap(o);
a(n).wrap(l);l=n.parent();o=l.parent();a(n).css("opacity",0).bind({"focus.uniform":function(){o.addClass(k.focusClass)
},"blur.uniform":function(){o.removeClass(k.focusClass)},"click.uniform touchend.uniform":function(){if(!a(n).attr("checked")){l.removeClass(k.checkedClass)
}else{l.addClass(k.checkedClass)}},"mousedown.uniform touchbegin.uniform":function(){o.addClass(k.activeClass)
},"mouseup.uniform touchend.uniform":function(){o.removeClass(k.activeClass)},"mouseenter.uniform":function(){o.addClass(k.hoverClass)
},"mouseleave.uniform":function(){o.removeClass(k.hoverClass);o.removeClass(k.activeClass)}});if(a(n).attr("checked")){l.addClass(k.checkedClass)
}if(a(n).attr("disabled")){o.addClass(k.disabledClass)}b(n)}function c(n){var m=a(n);var o=a("<div />"),l=a("<span />");
if(!m.css("display")=="none"&&k.autoHide){o.hide()}o.addClass(k.radioClass);if(k.useID&&n.attr("id")!=""){o.attr("id",k.idPrefix+"-"+n.attr("id"))
}a(n).wrap(o);a(n).wrap(l);l=n.parent();o=l.parent();a(n).css("opacity",0).bind({"focus.uniform":function(){o.addClass(k.focusClass)
},"blur.uniform":function(){o.removeClass(k.focusClass)},"click.uniform touchend.uniform":function(){if(!a(n).attr("checked")){l.removeClass(k.checkedClass)
}else{var p=k.radioClass.split(" ")[0];a("."+p+" span."+k.checkedClass+":has([name='"+a(n).attr("name")+"'])").removeClass(k.checkedClass);
l.addClass(k.checkedClass)}},"mousedown.uniform touchend.uniform":function(){if(!a(n).is(":disabled")){o.addClass(k.activeClass)
}},"mouseup.uniform touchbegin.uniform":function(){o.removeClass(k.activeClass)},"mouseenter.uniform touchend.uniform":function(){o.addClass(k.hoverClass)
},"mouseleave.uniform":function(){o.removeClass(k.hoverClass);o.removeClass(k.activeClass)}});if(a(n).attr("checked")){l.addClass(k.checkedClass)
}if(a(n).attr("disabled")){o.addClass(k.disabledClass)}b(n)}function h(q){var o=a(q);var r=a("<div />"),p=a("<span>"+k.fileDefaultText+"</span>"),m=a("<span>"+k.fileBtnText+"</span>");
if(!o.css("display")=="none"&&k.autoHide){r.hide()}r.addClass(k.fileClass);p.addClass(k.filenameClass);
m.addClass(k.fileBtnClass);if(k.useID&&o.attr("id")!=""){r.attr("id",k.idPrefix+"-"+o.attr("id"))}o.wrap(r);
o.after(m);o.after(p);r=o.closest("div");p=o.siblings("."+k.filenameClass);m=o.siblings("."+k.fileBtnClass);
if(!o.attr("size")){var l=r.width();o.attr("size",l/10)}var n=function(){var s=o.val();if(s===""){s=k.fileDefaultText
}else{s=s.split(/[\/\\]+/);s=s[(s.length-1)]}p.text(s)};n();o.css("opacity",0).bind({"focus.uniform":function(){r.addClass(k.focusClass)
},"blur.uniform":function(){r.removeClass(k.focusClass)},"mousedown.uniform":function(){if(!a(q).is(":disabled")){r.addClass(k.activeClass)
}},"mouseup.uniform":function(){r.removeClass(k.activeClass)},"mouseenter.uniform":function(){r.addClass(k.hoverClass)
},"mouseleave.uniform":function(){r.removeClass(k.hoverClass);r.removeClass(k.activeClass)}});if(a.browser.msie){o.bind("click.uniform.ie7",function(){setTimeout(n,0)
})}else{o.bind("change.uniform",n)}if(o.attr("disabled")){r.addClass(k.disabledClass)}a.uniform.noSelect(p);
a.uniform.noSelect(m);b(q)}a.uniform.restore=function(l){if(l==undefined){l=a(a.uniform.elements)}a(l).each(function(){if(a(this).is(":checkbox")){a(this).unwrap().unwrap()
}else{if(a(this).is("select")){a(this).siblings("span").remove();a(this).unwrap()}else{if(a(this).is(":radio")){a(this).unwrap().unwrap()
}else{if(a(this).is(":file")){a(this).siblings("span").remove();a(this).unwrap()}else{if(a(this).is("button, :submit, :reset, a, input[type='button']")){a(this).unwrap().unwrap()
}}}}}a(this).unbind(".uniform");a(this).css("opacity","1");var m=a.inArray(a(l),a.uniform.elements);a.uniform.elements.splice(m,1)
})};function b(l){l=a(l).get();if(l.length>1){a.each(l,function(m,n){a.uniform.elements.push(n)})}else{a.uniform.elements.push(l)
}}a.uniform.noSelect=function(l){function m(){return false}a(l).each(function(){this.onselectstart=this.ondragstart=m;
a(this).mousedown(m).css({MozUserSelect:"none"})})};a.uniform.update=function(l){if(l==undefined){l=a(a.uniform.elements)
}l=a(l);l.each(function(){var n=a(this);if(n.is("select")){var m=n.siblings("span");var p=n.parent("div");
p.removeClass(k.hoverClass+" "+k.focusClass+" "+k.activeClass);m.html(n.find(":selected").html());if(n.is(":disabled")){p.addClass(k.disabledClass)
}else{p.removeClass(k.disabledClass)}}else{if(n.is(":checkbox")){var m=n.closest("span");var p=n.closest("div");
p.removeClass(k.hoverClass+" "+k.focusClass+" "+k.activeClass);m.removeClass(k.checkedClass);if(n.is(":checked")){m.addClass(k.checkedClass)
}if(n.is(":disabled")){p.addClass(k.disabledClass)}else{p.removeClass(k.disabledClass)}}else{if(n.is(":radio")){var m=n.closest("span");
var p=n.closest("div");p.removeClass(k.hoverClass+" "+k.focusClass+" "+k.activeClass);m.removeClass(k.checkedClass);
if(n.is(":checked")){m.addClass(k.checkedClass)}if(n.is(":disabled")){p.addClass(k.disabledClass)}else{p.removeClass(k.disabledClass)
}}else{if(n.is(":file")){var p=n.parent("div");var o=n.siblings(k.filenameClass);btnTag=n.siblings(k.fileBtnClass);
p.removeClass(k.hoverClass+" "+k.focusClass+" "+k.activeClass);o.text(n.val());if(n.is(":disabled")){p.addClass(k.disabledClass)
}else{p.removeClass(k.disabledClass)}}else{if(n.is(":submit")||n.is(":reset")||n.is("button")||n.is("a")||l.is("input[type=button]")){var p=n.closest("div");
p.removeClass(k.hoverClass+" "+k.focusClass+" "+k.activeClass);if(n.is(":disabled")){p.addClass(k.disabledClass)
}else{p.removeClass(k.disabledClass)}}}}}}})};return this.each(function(){if(a.support.selectOpacity){var l=a(this);
if(l.is("select")){if(l.attr("multiple")!=true){if(l.attr("size")==undefined||l.attr("size")<=1){e(l)
}}}else{if(l.is(":checkbox")){f(l)}else{if(l.is(":radio")){c(l)}else{if(l.is(":file")){h(l)}else{if(l.is(":text, :password, input[type='email']")){j(l)
}else{if(l.is("textarea")){g(l)}else{if(l.is("a")||l.is(":submit")||l.is(":reset")||l.is("button")||l.is("input[type=button]")){i(l)
}}}}}}}}})}})(jQuery);(function(factory){if(typeof define==="function"&&define.amd){define(["jquery","jquery.ui.widget"],factory)
...
},"iframe script":function(iframe){return iframe&&$.globalEval($(iframe[0].body).text())}}})}));(function(){this.MooTools={version:"1.4.5",build:"ab8ea8824dc3b24b6666867a2c4ed58ebb762cf0"};
...
delete b[c]}}return this}})})();(function(){if(!window.attachEvent){return}var doc=document;var g=window.PIE;
if(!g){g=window.PIE={F:"-pie-",Sa:"Pie",Pa:"pie_",Jb:{TD:1,TH:1}};try{doc.execCommand("BackgroundImageCache",false,true)
}catch(L){}g.J=function(){for(var a=4,b=doc.createElement("div"),c=b.getElementsByTagName("i");b.innerHTML="<!--[if gt IE "+ ++a+"]><i></i><![endif]-->",c[0];
){}return a}();if(g.J===6){g.F=g.F.replace(/^-/,"")}g.Ab=doc.documentMode||g.J;(function(){var a,b=0,c={};
g.p={Ga:function(e){if(!a){a=doc.createDocumentFragment();a.namespaces.add("css3vml","urn:schemas-microsoft-com:vml")
}return a.createElement("css3vml:"+e)},ta:function(e){return e&&e._pieId||(e._pieId=++b)},fb:function(e){var f,h,j,d,i=arguments;
f=1;for(h=i.length;f<h;f++){d=i[f];for(j in d){if(d.hasOwnProperty(j)){e[j]=d[j]}}}return e},Pb:function(e,f,h){var j=c[e],d,i;
if(j){Object.prototype.toString.call(j)==="[object Array]"?j.push([f,h]):f.call(h,j)}else{i=c[e]=[[f,h]];
d=new Image;d.onload=function(){j=c[e]={i:d.width,f:d.height};for(var k=0,m=i.length;k<m;k++){i[k][0].call(i[k][1],j)
}d.onload=null};d.src=e}}}})();g.ia=function(){this.hb=[];this.Db={}};g.ia.prototype={aa:function(a){var b=g.p.ta(a),c=this.Db,e=this.hb;
if(!(b in c)){c[b]=e.length;e.push(a)}},Ma:function(a){a=g.p.ta(a);var b=this.Db;if(a&&a in b){delete this.hb[b[a]];
delete b[a]}},Ia:function(){for(var a=this.hb,b=a.length;b--;){a[b]&&a[b]()}}};g.ya=new g.ia;g.ya.Tc=function(){var a=this;
if(!a.Uc){setInterval(function(){a.Ia()},250);a.Uc=1}};g.G=new g.ia;window.attachEvent("onbeforeunload",function(){g.G.Ia()
});g.G.Ea=function(a,b,c){a.attachEvent(b,c);this.aa(function(){a.detachEvent(b,c)})};(function(){function a(){g.za.Ia()
}g.za=new g.ia;g.G.Ea(window,"onresize",a)})();(function(){function a(){g.Ra.Ia()}g.Ra=new g.ia;g.G.Ea(window,"onscroll",a);
g.za.aa(a)})();(function(){function a(){c=g.Qa.wc()}function b(){if(c){for(var e=0,f=c.length;e<f;e++){g.attach(c[e])
}c=0}}var c;g.G.Ea(window,"onbeforeprint",a);g.G.Ea(window,"onafterprint",b)})();g.hd=function(){function a(i){this.V=i
}var b=doc.createElement("length-calc"),c=doc.documentElement,e=b.style,f={},h=["mm","cm","in","pt","pc"],j=h.length,d={};
e.position="absolute";e.top=e.left="-9999px";for(c.appendChild(b);j--;){b.style.width="100"+h[j];f[h[j]]=b.offsetWidth/100
}c.removeChild(b);a.prototype={ib:/(px|em|ex|mm|cm|in|pt|pc|%)$/,vb:function(){var i=this.Lc;if(i===void 0){i=this.Lc=parseFloat(this.V)
}return i},ab:function(){var i=this.ad;if(!i){i=this.ad=(i=this.V.match(this.ib))&&i[0]||"px"}return i
},a:function(i,k){var m=this.vb(),l=this.ab();switch(l){case"px":return m;case"%":return m*(typeof k==="function"?k():k)/100;
case"em":return m*this.tb(i);case"ex":return m*this.tb(i)/2;default:return m*f[l]}},tb:function(i){var k=i.currentStyle.fontSize;
if(k.indexOf("px")>0){return parseFloat(k)}else{b.style.width="1em";i.appendChild(b);k=b.offsetWidth;
b.parentNode===i&&i.removeChild(b);return k}}};g.k=function(i){return d[i]||(d[i]=new a(i))};return a
}();g.Na=function(){function a(f){this.U=f}var b=g.k("50%"),c={top:1,center:1,bottom:1},e={left:1,center:1,right:1};
a.prototype={Dc:function(){if(!this.sb){var f=this.U,h=f.length,j=g.u,d=j.ja,i=g.k("0");d=d.fa;i=["left",i,"top",i];
if(h===1){f.push(new j.Ta(d,"center"));h++}if(h===2){d&(f[0].h|f[1].h)&&f[0].d in c&&f[1].d in e&&f.push(f.shift());
if(f[0].h&d){if(f[0].d==="center"){i[1]=b}else{i[0]=f[0].d}}else{if(f[0].Y()){i[1]=g.k(f[0].d)}}if(f[1].h&d){if(f[1].d==="center"){i[3]=b
}else{i[2]=f[1].d}}else{if(f[1].Y()){i[3]=g.k(f[1].d)}}}this.sb=i}return this.sb},coords:function(f,h,j){var d=this.Dc(),i=d[1].a(f,h);
f=d[3].a(f,j);return{x:d[0]==="right"?h-i:i,y:d[2]==="bottom"?j-f:f}}};return a}();g.Rb=function(){function a(b){this.V=b
}a.prototype={ib:/[a-z]+$/i,ab:function(){return this.lc||(this.lc=this.V.match(this.ib)[0].toLowerCase())
},vc:function(){var b=this.fc,c;if(b===undefined){b=this.ab();c=parseFloat(this.V,10);b=this.fc=b==="deg"?c:b==="rad"?c/Math.PI*180:b==="grad"?c/400*360:b==="turn"?c*360:0
}return b}};return a}();g.$b=function(){function a(c){this.V=c}var b={};a.Sc=/\s*rgba\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d+|\d*\.\d+)\s*\)\s*/;
a.gb={aliceblue:"F0F8FF",antiquewhite:"FAEBD7",aqua:"0FF",aquamarine:"7FFFD4",azure:"F0FFFF",beige:"F5F5DC",bisque:"FFE4C4",black:"000",blanchedalmond:"FFEBCD",blue:"00F",blueviolet:"8A2BE2",brown:"A52A2A",burlywood:"DEB887",cadetblue:"5F9EA0",chartreuse:"7FFF00",chocolate:"D2691E",coral:"FF7F50",cornflowerblue:"6495ED",cornsilk:"FFF8DC",crimson:"DC143C",cyan:"0FF",darkblue:"00008B",darkcyan:"008B8B",darkgoldenrod:"B8860B",darkgray:"A9A9A9",darkgreen:"006400",darkkhaki:"BDB76B",darkmagenta:"8B008B",darkolivegreen:"556B2F",darkorange:"FF8C00",darkorchid:"9932CC",darkred:"8B0000",darksalmon:"E9967A",darkseagreen:"8FBC8F",darkslateblue:"483D8B",darkslategray:"2F4F4F",darkturquoise:"00CED1",darkviolet:"9400D3",deeppink:"FF1493",deepskyblue:"00BFFF",dimgray:"696969",dodgerblue:"1E90FF",firebrick:"B22222",floralwhite:"FFFAF0",forestgreen:"228B22",fuchsia:"F0F",gainsboro:"DCDCDC",ghostwhite:"F8F8FF",gold:"FFD700",goldenrod:"DAA520",gray:"808080",green:"008000",greenyellow:"ADFF2F",honeydew:"F0FFF0",hotpink:"FF69B4",indianred:"CD5C5C",indigo:"4B0082",ivory:"FFFFF0",khaki:"F0E68C",lavender:"E6E6FA",lavenderblush:"FFF0F5",lawngreen:"7CFC00",lemonchiffon:"FFFACD",lightblue:"ADD8E6",lightcoral:"F08080",lightcyan:"E0FFFF",lightgoldenrodyellow:"FAFAD2",lightgreen:"90EE90",lightgrey:"D3D3D3",lightpink:"FFB6C1",lightsalmon:"FFA07A",lightseagreen:"20B2AA",lightskyblue:"87CEFA",lightslategray:"789",lightsteelblue:"B0C4DE",lightyellow:"FFFFE0",lime:"0F0",limegreen:"32CD32",linen:"FAF0E6",magenta:"F0F",maroon:"800000",mediumauqamarine:"66CDAA",mediumblue:"0000CD",mediumorchid:"BA55D3",mediumpurple:"9370D8",mediumseagreen:"3CB371",mediumslateblue:"7B68EE",mediumspringgreen:"00FA9A",mediumturquoise:"48D1CC",mediumvioletred:"C71585",midnightblue:"191970",mintcream:"F5FFFA",mistyrose:"FFE4E1",moccasin:"FFE4B5",navajowhite:"FFDEAD",navy:"000080",oldlace:"FDF5E6",olive:"808000",olivedrab:"688E23",orange:"FFA500",orangered:"FF4500",orchid:"DA70D6",palegoldenrod:"EEE8AA",palegreen:"98FB98",paleturquoise:"AFEEEE",palevioletred:"D87093",papayawhip:"FFEFD5",peachpuff:"FFDAB9",peru:"CD853F",pink:"FFC0CB",plum:"DDA0DD",powderblue:"B0E0E6",purple:"800080",red:"F00",rosybrown:"BC8F8F",royalblue:"4169E1",saddlebrown:"8B4513",salmon:"FA8072",sandybrown:"F4A460",seagreen:"2E8B57",seashell:"FFF5EE",sienna:"A0522D",silver:"C0C0C0",skyblue:"87CEEB",slateblue:"6A5ACD",slategray:"708090",snow:"FFFAFA",springgreen:"00FF7F",steelblue:"4682B4",tan:"D2B48C",teal:"008080",thistle:"D8BFD8",tomato:"FF6347",turquoise:"40E0D0",violet:"EE82EE",wheat:"F5DEB3",white:"FFF",whitesmoke:"F5F5F5",yellow:"FF0",yellowgreen:"9ACD32"};
a.prototype={parse:function(){if(!this.Ca){var c=this.V,e;if(e=c.match(a.Sc)){this.Ca="rgb("+e[1]+","+e[2]+","+e[3]+")";
this.qb=parseFloat(e[4])}else{if((e=c.toLowerCase()) in a.gb){c="#"+a.gb[e]}this.Ca=c;this.qb=c==="transparent"?0:1
}}},O:function(c){this.parse();return this.Ca==="currentColor"?c.currentStyle.color:this.Ca},la:function(){this.parse();
return this.qb}};g.pa=function(c){return b[c]||(b[c]=new a(c))};return a}();g.u=function(){function a(c){this.Ha=c;
this.ch=0;this.U=[];this.wa=0}var b=a.ja={xa:1,ob:2,ea:4,ac:8,pb:16,fa:32,A:64,ga:128,ha:256,Aa:512,dc:1024,URL:2048};
a.Ta=function(c,e){this.h=c;this.d=e};a.Ta.prototype={db:function(){return this.h&b.A||this.h&b.ga&&this.d==="0"
},Y:function(){return this.db()||this.h&b.Aa}};a.prototype={dd:/\s/,Mc:/^[\+\-]?(\d*\.)?\d+/,url:/^url\(\s*("([^"]*)"|'([^']*)'|([!#$%&*-~]*))\s*\)/i,zb:/^\-?[_a-z][\w-]*/i,Yc:/^("([^"]*)"|'([^']*)')/,Fc:/^#([\da-f]{6}|[\da-f]{3})/i,bd:{px:b.A,em:b.A,ex:b.A,mm:b.A,cm:b.A,"in":b.A,pt:b.A,pc:b.A,deg:b.xa,rad:b.xa,grad:b.xa},sc:{rgb:1,rgba:1,hsl:1,hsla:1},next:function(c){function e(t,n){t=new a.Ta(t,n);
if(!c){k.U.push(t);k.wa++}return t}function f(){k.wa++;return null}var h,j,d,i,k=this;if(this.wa<this.U.length){return this.U[this.wa++]
}for(;this.dd.test(this.Ha.charAt(this.ch));){this.ch++}if(this.ch>=this.Ha.length){return f()}j=this.ch;
h=this.Ha.substring(this.ch);d=h.charAt(0);switch(d){case"#":if(i=h.match(this.Fc)){this.ch+=i[0].length;
return e(b.ea,i[0])}break;case'"':case"'":if(i=h.match(this.Yc)){this.ch+=i[0].length;return e(b.dc,i[2]||i[3]||"")
}break;case"/":case",":this.ch++;return e(b.ha,d);case"u":if(i=h.match(this.url)){this.ch+=i[0].length;
return e(b.URL,i[2]||i[3]||i[4]||"")}}if(i=h.match(this.Mc)){d=i[0];this.ch+=d.length;if(h.charAt(d.length)==="%"){this.ch++;
return e(b.Aa,d+"%")}if(i=h.substring(d.length).match(this.zb)){d+=i[0];this.ch+=i[0].length;return e(this.bd[i[0].toLowerCase()]||b.ac,d)
}return e(b.ga,d)}if(i=h.match(this.zb)){d=i[0];this.ch+=d.length;if(d.toLowerCase() in g.$b.gb||d==="currentColor"){return e(b.ea,d)
}if(h.charAt(d.length)==="("){this.ch++;if(d.toLowerCase() in this.sc){h=function(t){return t&&t.h&b.ga
};i=function(t){return t&&t.h&(b.ga|b.Aa)};var m=function(t,n){return t&&t.d===n},l=function(){return k.next(1)
};if((d.charAt(0)==="r"?i(l()):h(l()))&&m(l(),",")&&i(l())&&m(l(),",")&&i(l())&&(d==="rgb"||d==="hsa"||m(l(),",")&&h(l()))&&m(l(),")")){return e(b.ea,this.Ha.substring(j,this.ch))
}return f()}return e(b.pb,d)}return e(b.fa,d)}this.ch++;return e(b.ob,d)},z:function(){return this.U[this.wa---2]
},all:function(){for(;this.next();){}return this.U},da:function(c,e){for(var f=[],h,j;h=this.next();){if(c(h)){j=true;
this.z();break}f.push(h)}return e&&!j?null:f}};return a}();var M=function(a){this.e=a};M.prototype={K:0,Qc:function(){var a=this.Ua,b;
return !a||(b=this.o())&&(a.x!==b.x||a.y!==b.y)},Vc:function(){var a=this.Ua,b;return !a||(b=this.o())&&(a.i!==b.i||a.f!==b.f)
},ub:function(){var a=this.e.getBoundingClientRect();return{x:a.left,y:a.top,i:a.right-a.left,f:a.bottom-a.top}
},o:function(){return this.K?this.Da||(this.Da=this.ub()):this.ub()},Ec:function(){return !!this.Ua},Ja:function(){++this.K
},La:function(){if(!--this.K){if(this.Da){this.Ua=this.Da}this.Da=null}}};(function(){function a(b){var c=g.p.ta(b);
return function(){if(this.K){var e=this.rb||(this.rb={});return c in e?e[c]:(e[c]=b.call(this))}else{return b.call(this)
}}}g.s={K:0,$:function(b){function c(e){this.e=e}g.p.fb(c.prototype,g.s,b);c.kc={};return c},m:function(){var b=this.qa(),c=this.constructor.kc;
return b?b in c?c[b]:(c[b]=this.ba(b)):null},qa:a(function(){var b=this.e,c=this.constructor,e=b.style;
b=b.currentStyle;var f=this.na,h=this.va,j=c.ic||(c.ic=g.F+f);c=c.jc||(c.jc=g.Sa+h.charAt(0).toUpperCase()+h.substring(1));
return e[c]||b.getAttribute(j)||e[h]||b.getAttribute(f)}),g:a(function(){return !!this.m()}),D:a(function(){var b=this.qa(),c=b!==this.gc;
this.gc=b;return c}),ma:a,Ja:function(){++this.K},La:function(){--this.K||delete this.rb}}})();g.Tb=g.s.$({na:g.F+"background",va:g.Sa+"Background",nc:{scroll:1,fixed:1,local:1},Ka:{"repeat-x":1,"repeat-y":1,repeat:1,"no-repeat":1},Nc:{"padding-box":1,"border-box":1,"content-box":1},rc:{"padding-box":1,"border-box":1},Rc:{top:1,right:1,bottom:1,left:1,center:1},Wc:{contain:1,cover:1},ba:function(a){function b(u){return u.Y()||u.h&i&&u.d in t
}function c(u){return u.Y()&&g.k(u.d)||u.d==="auto"&&"auto"}var e=this.e.currentStyle,f,h,j=g.u.ja,d=j.ha,i=j.fa,k=j.ea,m,l,t=this.Rc,n,p,s=null;
if(this.$a()){a=new g.u(a);s={M:[]};for(h={};f=a.next();){m=f.h;l=f.d;if(!h.P&&m&j.pb&&l==="linear-gradient"){n={ca:[],P:l};
for(p={};f=a.next();){m=f.h;l=f.d;if(m&j.ob&&l===")"){p.color&&n.ca.push(p);n.ca.length>1&&g.p.fb(h,n);
break}if(m&k){if(n.Xa||n.bb){f=a.z();if(f.h!==d){break}a.next()}p={color:g.pa(l)};f=a.next();if(f.Y()){p.Fb=g.k(f.d)
}else{a.z()}}else{if(m&j.xa&&!n.Xa&&!p.color&&!n.ca.length){n.Xa=new g.Rb(f.d)}else{if(b(f)&&!n.bb&&!p.color&&!n.ca.length){a.z();
n.bb=new g.Na(a.da(function(u){return !b(u)},false))}else{if(m&d&&l===","){if(p.color){n.ca.push(p);p={}
}}else{break}}}}}}else{if(!h.P&&m&j.URL){h.Cb=l;h.P="image"}else{if(b(f)&&!h.size){a.z();h.Ya=new g.Na(a.da(function(u){return !b(u)
},false))}else{if(m&i){if(l in this.Ka){h.Bb=l}else{if(l in this.Nc){h.kd=l;if(l in this.rc){h.clip=l
}}else{if(l in this.nc){h.jd=l}}}}else{if(m&k&&!s.color){s.color=g.pa(l)}else{if(m&d){if(l==="/"){f=a.next();
m=f.h;l=f.d;if(m&i&&l in this.Wc){h.size=l}else{if(l=c(f)){h.size={i:l,f:c(a.next())||a.z()&&l}}}}else{if(l===","&&h.P){s.M.push(h);
h={}}}}else{return null}}}}}}}h.P&&s.M.push(h)}else{this.Nb(function(){var u=e.backgroundPositionX,w=e.backgroundPositionY,r=e.backgroundImage,o=e.backgroundColor;
s={};if(o!=="transparent"){s.color=g.pa(o)}if(r!=="none"){s.M=[{P:"image",Cb:(new g.u(r)).next().d,Bb:e.backgroundRepeat,Ya:new g.Na((new g.u(u+" "+w)).all())}]
}})}return s&&(s.color||s.M&&s.M[0])?s:null},Nb:function(a){var b=this.e.runtimeStyle,c=b.backgroundImage,e=b.backgroundColor;
if(c){b.backgroundImage=""}if(e){b.backgroundColor=""}a=a.call(this);if(c){b.backgroundImage=c}if(e){b.backgroundColor=e
}return a},qa:g.s.ma(function(){return this.$a()||this.Nb(function(){var a=this.e.currentStyle;return a.backgroundColor+" "+a.backgroundImage+" "+a.backgroundRepeat+" "+a.backgroundPositionX+" "+a.backgroundPositionY
})}),$a:g.s.ma(function(){var a=this.e;return a.style[this.va]||a.currentStyle.getAttribute(this.na)}),Eb:function(){var a=0;
if(g.J<7){a=this.e;a=""+(a.style[g.Sa+"PngFix"]||a.currentStyle.getAttribute(g.F+"png-fix"))==="true"
}return a},g:g.s.ma(function(){return(this.$a()||this.Eb())&&!!this.m()})});g.Xb=g.s.$({Ib:["Top","Right","Bottom","Left"],Kc:{thin:"1px",medium:"3px",thick:"5px"},ba:function(){var a={},b={},c={},e=false,f=true,h=true,j=true;
this.Ob(function(){for(var d=this.e.currentStyle,i=0,k,m,l,t,n,p,s;i<4;i++){l=this.Ib[i];s=l.charAt(0).toLowerCase();
k=b[s]=d["border"+l+"Style"];m=d["border"+l+"Color"];l=d["border"+l+"Width"];if(i>0){if(k!==t){h=false
}if(m!==n){f=false}if(l!==p){j=false}}t=k;n=m;p=l;c[s]=g.pa(m);l=a[s]=g.k(b[s]==="none"?"0":this.Kc[l]||l);
if(l.a(this.e)>0){e=true}}});return e?{nb:a,Zc:b,tc:c,ed:j,uc:f,$c:h}:null},qa:g.s.ma(function(){var a=this.e,b=a.currentStyle,c;
a.tagName in g.Jb&&a.offsetParent.currentStyle.borderCollapse==="collapse"||this.Ob(function(){c=b.borderWidth+"|"+b.borderStyle+"|"+b.borderColor
});return c}),Ob:function(a){var b=this.e.runtimeStyle,c=b.borderWidth,e=b.borderColor;if(c){b.borderWidth=""
}if(e){b.borderColor=""}a=a.call(this);if(c){b.borderWidth=c}if(e){b.borderColor=e}return a}});(function(){g.Oa=g.s.$({na:"border-radius",va:"borderRadius",ba:function(b){var c=null,e,f,h,j,d=false;
if(b){f=new g.u(b);var i=function(){for(var k=[],m;(h=f.next())&&h.Y();){j=g.k(h.d);m=j.vb();if(m<0){return null
}if(m>0){d=true}k.push(j)}return k.length>0&&k.length<5?{tl:k[0],tr:k[1]||k[0],br:k[2]||k[0],bl:k[3]||k[1]||k[0]}:null
};if(b=i()){if(h){if(h.h&g.u.ja.ha&&h.d==="/"){e=i()}}else{e=b}if(d&&b&&e){c={x:b,y:e}}}}return c}});
var a=g.k("0");a={tl:a,tr:a,br:a,bl:a};g.Oa.Qb={x:a,y:a}})();g.Vb=g.s.$({na:"border-image",va:"borderImage",Ka:{stretch:1,round:1,repeat:1,space:1},ba:function(a){var b=null,c,e,f,h,j,d,i=0,k,m=g.u.ja,l=m.fa,t=m.ga,n=m.A,p=m.Aa;
if(a){c=new g.u(a);b={};for(var s=function(r){return r&&r.h&m.ha&&r.d==="/"},u=function(r){return r&&r.h&l&&r.d==="fill"
},w=function(){h=c.da(function(r){return !(r.h&(t|p))});if(u(c.next())&&!b.fill){b.fill=true}else{c.z()
}if(s(c.next())){i++;j=c.da(function(){return !(e.h&(t|p|n))&&!(e.h&l&&e.d==="auto")});if(s(c.next())){i++;
d=c.da(function(){return !(e.h&(t|n))})}}else{c.z()}};e=c.next();){a=e.h;f=e.d;if(a&(t|p)&&!h){c.z();
w()}else{if(u(e)&&!b.fill){b.fill=true;w()}else{if(a&l&&this.Ka[f]&&!b.repeat){b.repeat={f:f};if(e=c.next()){if(e.h&l&&this.Ka[e.d]){b.repeat.kb=e.d
}else{c.z()}}}else{if(a&m.URL&&!b.src){b.src=f}else{return null}}}}}if(!b.src||!h||h.length<1||h.length>4||j&&j.length>4||i===1&&j.length<1||d&&d.length>4||i===2&&d.length<1){return null
}if(!b.repeat){b.repeat={f:"stretch"}}if(!b.repeat.kb){b.repeat.kb=b.repeat.f}a=function(r,o){return{T:o(r[0]),S:o(r[1]||r[0]),L:o(r[2]||r[0]),Q:o(r[3]||r[1]||r[0])}
};b.slice=a(h,function(r){return g.k(r.h&t?r.d+"px":r.d)});b.width=j&&j.length>0?a(j,function(r){return r.h&(n|p)?g.k(r.d):r.d
}):(k=this.e.currentStyle)&&{T:g.k(k.borderTopWidth),S:g.k(k.borderRightWidth),L:g.k(k.borderBottomWidth),Q:g.k(k.borderLeftWidth)};
b.ua=a(d||[0],function(r){return r.h&n?g.k(r.d):r.d})}return b}});g.Zb=g.s.$({na:"box-shadow",va:"boxShadow",ba:function(a){var b,c=g.k,e=g.u.ja,f;
if(a){f=new g.u(a);b={ua:[],cb:[]};for(a=function(){for(var h,j,d,i,k,m;h=f.next();){d=h.d;j=h.h;if(j&e.ha&&d===","){break
}else{if(h.db()&&!k){f.z();k=f.da(function(l){return !l.db()})}else{if(j&e.ea&&!i){i=d}else{if(j&e.fa&&d==="inset"&&!m){m=true
}else{return false}}}}}h=k&&k.length;if(h>1&&h<5){(m?b.cb:b.ua).push({fd:c(k[0].d),gd:c(k[1].d),blur:c(k[2]?k[2].d:"0"),Xc:c(k[3]?k[3].d:"0"),color:g.pa(i||"currentColor")});
return true}return false};a();){}}return b&&(b.cb.length||b.ua.length)?b:null}});g.ec=g.s.$({qa:g.s.ma(function(){var a=this.e.currentStyle;
return a.visibility+"|"+a.display}),ba:function(){var a=this.e,b=a.runtimeStyle;a=a.currentStyle;var c=b.visibility,e;
b.visibility="";e=a.visibility;b.visibility=c;return{cd:e!=="hidden",xc:a.display!=="none"}},g:function(){return false
}});g.B={Z:function(a){function b(c,e,f,h){this.e=c;this.q=e;this.j=f;this.parent=h}g.p.fb(b.prototype,g.B,a);
return b},eb:false,R:function(){return false},Kb:function(){this.n();this.g()&&this.X()},jb:function(){this.eb=true
},Lb:function(){this.g()?this.X():this.n()},Wa:function(a,b){this.Hb(a);for(var c=this.ka||(this.ka=[]),e=a+1,f=c.length,h;
e<f;e++){if(h=c[e]){break}}c[a]=b;this.w().insertBefore(b,h||null)},ra:function(a){var b=this.ka;return b&&b[a]||null
},Hb:function(a){var b=this.ra(a),c=this.Ba;if(b&&c){c.removeChild(b);this.ka[a]=null}},sa:function(a,b,c,e){var f=this.Va||(this.Va={}),h=f[a];
if(!h){h=f[a]=g.p.Ga("shape");if(b){h.appendChild(h[b]=g.p.Ga(b))}if(e){c=this.ra(e);if(!c){this.Wa(e,doc.createElement("group"+e));
c=this.ra(e)}}c.appendChild(h);a=h.style;a.position="absolute";a.left=a.top=0;a.behavior="url(#default#VML)"
}return h},Za:function(a){var b=this.Va,c=b&&b[a];if(c){c.parentNode.removeChild(c);delete b[a]}return !!c
},xb:function(a){var b=this.e,c=this.q.o(),e=c.i,f=c.f,h,j,d,i,k,m;c=a.x.tl.a(b,e);h=a.y.tl.a(b,f);j=a.x.tr.a(b,e);
d=a.y.tr.a(b,f);i=a.x.br.a(b,e);k=a.y.br.a(b,f);m=a.x.bl.a(b,e);a=a.y.bl.a(b,f);e=Math.min(e/(c+j),f/(d+k),e/(m+i),f/(h+a));
if(e<1){c*=e;h*=e;j*=e;d*=e;i*=e;k*=e;m*=e;a*=e}return{x:{tl:c,tr:j,br:i,bl:m},y:{tl:h,tr:d,br:k,bl:a}}
},oa:function(a,b,c){b=b||1;var e,f,h=this.q.o();f=h.i*b;h=h.f*b;var j=this.j.v,d=Math.floor,i=Math.ceil,k=a?a.T*b:0,m=a?a.S*b:0,l=a?a.L*b:0;
a=a?a.Q*b:0;var t,n,p,s,u;if(c||j.g()){e=this.xb(c||j.m());c=e.x.tl*b;j=e.y.tl*b;t=e.x.tr*b;n=e.y.tr*b;
p=e.x.br*b;s=e.y.br*b;u=e.x.bl*b;b=e.y.bl*b;f="m"+d(a)+","+d(j)+"qy"+d(c)+","+d(k)+"l"+i(f-t)+","+d(k)+"qx"+i(f-m)+","+d(n)+"l"+i(f-m)+","+i(h-s)+"qy"+i(f-p)+","+i(h-l)+"l"+d(u)+","+i(h-l)+"qx"+d(a)+","+i(h-b)+" x e"
}else{f="m"+d(a)+","+d(k)+"l"+i(f-m)+","+d(k)+"l"+i(f-m)+","+i(h-l)+"l"+d(a)+","+i(h-l)+"xe"}return f
},w:function(){var a=this.parent.ra(this.C),b;if(!a){a=doc.createElement(this.Fa);b=a.style;b.position="absolute";
b.top=b.left=0;this.parent.Wa(this.C,a)}return a},n:function(){this.parent.Hb(this.C);delete this.Va;
delete this.ka}};g.cc=g.B.Z({g:function(){var a=this.oc;for(var b in a){if(a.hasOwnProperty(b)&&a[b].g()){return true
}}return false},R:function(){return this.j.lb.D()},jb:function(){if(this.g()){var a=this.wb(),b=a,c;a=a.currentStyle;
var e=a.position,f=this.w().style,h=0,j=0;j=this.q.o();if(e==="fixed"&&g.J>6){h=j.x;j=j.y;b=e}else{do{b=b.offsetParent
}while(b&&b.currentStyle.position==="static");if(b){c=b.getBoundingClientRect();b=b.currentStyle;h=j.x-c.left-(parseFloat(b.borderLeftWidth)||0);
j=j.y-c.top-(parseFloat(b.borderTopWidth)||0)}else{b=doc.documentElement;h=j.x+b.scrollLeft-b.clientLeft;
j=j.y+b.scrollTop-b.clientTop}b="absolute"}f.position=b;f.left=h;f.top=j;f.zIndex=e==="static"?-1:a.zIndex;
this.eb=true}},Lb:function(){},Mb:function(){var a=this.j.lb.m();this.w().style.display=a.cd&&a.xc?"":"none"
},Kb:function(){this.g()?this.Mb():this.n()},wb:function(){var a=this.e;return a.tagName in g.Jb?a.offsetParent:a
},w:function(){var a=this.Ba,b;if(!a){b=this.wb();a=this.Ba=doc.createElement("css3-container");a.style.direction="ltr";
this.Mb();b.parentNode.insertBefore(a,b)}return a},n:function(){var a=this.Ba,b;if(a&&(b=a.parentNode)){b.removeChild(a)
}delete this.Ba;delete this.ka}});g.Sb=g.B.Z({C:2,Fa:"background",R:function(){var a=this.j;return a.H.D()||a.v.D()
},g:function(){var a=this.j;return a.N.g()||a.v.g()||a.H.g()||a.W.g()&&a.W.m().cb},X:function(){var a=this.q.o();
if(a.i&&a.f){this.yc();this.zc()}},yc:function(){var a=this.j.H.m(),b=this.q.o(),c=this.e,e=a&&a.color,f,h;
if(e&&e.la()>0){this.yb();a=this.sa("bgColor","fill",this.w(),1);f=b.i;b=b.f;a.stroked=false;a.coordsize=f*2+","+b*2;
a.coordorigin="1,1";a.path=this.oa(null,2);h=a.style;h.width=f;h.height=b;a.fill.color=e.O(c);c=e.la();
if(c<1){a.fill.opacity=c}}else{this.Za("bgColor")}},zc:function(){var a=this.j.H.m(),b=this.q.o();a=a&&a.M;
var c,e,f,h,j;if(a){this.yb();e=b.i;f=b.f;for(j=a.length;j--;){b=a[j];c=this.sa("bgImage"+j,"fill",this.w(),2);
c.stroked=false;c.fill.type="tile";c.fillcolor="none";c.coordsize=e*2+","+f*2;c.coordorigin="1,1";c.path=this.oa(0,2);
h=c.style;h.width=e;h.height=f;if(b.P==="linear-gradient"){this.mc(c,b)}else{c.fill.src=b.Cb;this.Pc(c,j)
}}}for(j=a?a.length:0;this.Za("bgImage"+j++);){}},Pc:function(a,b){g.p.Pb(a.fill.src,function(c){var e=a.fill,f=this.e,h=this.q.o(),j=h.i;
h=h.f;var d=this.j,i=d.I.m(),k=i&&i.nb;i=k?k.t.a(f):0;var m=k?k.r.a(f):0,l=k?k.b.a(f):0;k=k?k.l.a(f):0;
d=d.H.m().M[b];f=d.Ya?d.Ya.coords(f,j-c.i-k-m,h-c.f-i-l):{x:0,y:0};d=d.Bb;l=m=0;var t=j+1,n=h+1,p=g.J===8?0:1;
k=Math.round(f.x)+k+0.5;i=Math.round(f.y)+i+0.5;e.position=k/j+","+i/h;if(d&&d!=="repeat"){if(d==="repeat-x"||d==="no-repeat"){m=i+1;
n=i+c.f+p}if(d==="repeat-y"||d==="no-repeat"){l=k+1;t=k+c.i+p}a.style.clip="rect("+m+"px,"+t+"px,"+n+"px,"+l+"px)"
}},this)},mc:function(a,b){function c(B,C,z,F,H){if(z===0||z===180){return[F,C]}else{if(z===90||z===270){return[B,H]
}else{z=Math.tan(-z*t/180);B=z*B-C;C=-1/z;F=C*F-H;H=C-z;return[(F-B)/H,(z*F-C*B)/H]}}}function e(){w=m>=90&&m<270?i:0;
r=m<180?k:0;o=i-w;x=k-r}function f(){for(;m<0;){m+=360}m%=360}function h(B,C){var z=C[0]-B[0];B=C[1]-B[1];
return Math.abs(z===0?B:B===0?z:Math.sqrt(z*z+B*B))}var j=this.e,d=this.q.o(),i=d.i,k=d.f;a=a.fill;var m=b.Xa,l=b.bb;
b=b.ca;d=b.length;var t=Math.PI,n,p,s,u,w,r,o,x,q,y,A,D;if(l){l=l.coords(j,i,k);n=l.x;p=l.y}if(m){m=m.vc();
f();e();if(!l){n=w;p=r}l=c(n,p,m,o,x);s=l[0];u=l[1]}else{if(l){s=i-n;u=k-p}else{n=p=s=0;u=k}}l=s-n;q=u-p;
if(m===void 0){m=!l?q<0?90:270:!q?l<0?180:0:-Math.atan2(q,l)/t*180;f();e()}l=m%90?Math.atan2(l*i/k,q)/t*180:m+90;
l+=180;l%=360;y=h([n,p],[s,u]);s=h([w,r],c(w,r,m,o,x));u=[];p=h([n,p],c(n,p,m,w,r))/s*100;n=[];for(q=0;
q<d;q++){n.push(b[q].Fb?b[q].Fb.a(j,y):q===0?0:q===d-1?y:null)}for(q=1;q<d;q++){if(n[q]===null){A=n[q-1];
y=q;do{D=n[++y]}while(D===null);n[q]=A+(D-A)/(y-q+1)}n[q]=Math.max(n[q],n[q-1])}for(q=0;q<d;q++){u.push(p+n[q]/s*100+"% "+b[q].color.O(j))
}a.angle=l;a.type="gradient";a.method="sigma";a.color=b[0].color.O(j);a.color2=b[d-1].color.O(j);a.colors.value=u.join(",")
},yb:function(){var a=this.e.runtimeStyle;a.backgroundImage="url(about:blank)";a.backgroundColor="transparent"
},n:function(){g.B.n.call(this);var a=this.e.runtimeStyle;a.backgroundImage=a.backgroundColor=""}});g.Wb=g.B.Z({C:4,Fa:"border",qc:{TABLE:1,INPUT:1,TEXTAREA:1,SELECT:1,OPTION:1,IMG:1,HR:1,FIELDSET:1},Jc:{submit:1,button:1,reset:1},R:function(){var a=this.j;
return a.I.D()||a.v.D()},g:function(){var a=this.j;return(a.N.g()||a.v.g()||a.H.g())&&a.I.g()},X:function(){var a=this.e,b=this.j.I.m(),c=this.q.o(),e=c.i;
c=c.f;var f,h,j,d,i;if(b){this.Hc();b=this.Bc(2);d=0;for(i=b.length;d<i;d++){j=b[d];f=this.sa("borderPiece"+d,j.stroke?"stroke":"fill",this.w());
f.coordsize=e*2+","+c*2;f.coordorigin="1,1";f.path=j.path;h=f.style;h.width=e;h.height=c;f.filled=!!j.fill;
f.stroked=!!j.stroke;if(j.stroke){f=f.stroke;f.weight=j.mb+"px";f.color=j.color.O(a);f.dashstyle=j.stroke==="dashed"?"2 2":j.stroke==="dotted"?"1 1":"solid";
f.linestyle=j.stroke==="double"&&j.mb>2?"ThinThin":"Single"}else{f.fill.color=j.fill.O(a)}}for(;this.Za("borderPiece"+d++);
){}}},Hc:function(){var a=this.e,b=a.currentStyle,c=a.runtimeStyle,e=a.tagName,f=g.J===6,h;if(f&&e in this.qc||e==="BUTTON"||e==="INPUT"&&a.type in this.Jc){c.borderWidth="";
e=this.j.I.Ib;for(h=e.length;h--;){f=e[h];c["padding"+f]="";c["padding"+f]=g.k(b["padding"+f]).a(a)+g.k(b["border"+f+"Width"]).a(a)+(!g.J===8&&h%2?1:0)
}c.borderWidth=0}else{if(f){if(a.childNodes.length!==1||a.firstChild.tagName!=="ie6-mask"){b=doc.createElement("ie6-mask");
e=b.style;e.visibility="visible";for(e.zoom=1;e=a.firstChild;){b.appendChild(e)}a.appendChild(b);c.visibility="hidden"
}}else{c.borderColor="transparent"}}},Bc:function(a){var b=this.e,c,e,f,h=this.j.I,j=[],d,i,k,m,l=Math.round,t,n,p;
if(h.g()){c=h.m();h=c.nb;n=c.Zc;p=c.tc;if(c.ed&&c.$c&&c.uc){if(p.t.la()>0){c=h.t.a(b);k=c/2;j.push({path:this.oa({T:k,S:k,L:k,Q:k},a),stroke:n.t,color:p.t,mb:c})
}}else{a=a||1;c=this.q.o();e=c.i;f=c.f;c=l(h.t.a(b));k=l(h.r.a(b));m=l(h.b.a(b));b=l(h.l.a(b));var s={t:c,r:k,b:m,l:b};
b=this.j.v;if(b.g()){t=this.xb(b.m())}d=Math.floor;i=Math.ceil;var u=function(o,x){return t?t[o][x]:0
},w=function(o,x,q,y,A,D){var B=u("x",o),C=u("y",o),z=o.charAt(1)==="r";o=o.charAt(0)==="b";return B>0&&C>0?(D?"al":"ae")+(z?i(e-B):d(B))*a+","+(o?i(f-C):d(C))*a+","+(d(B)-x)*a+","+(d(C)-q)*a+","+y*65535+","+2949075*(A?1:-1):(D?"m":"l")+(z?e-x:x)*a+","+(o?f-q:q)*a
},r=function(o,x,q,y){var A=o==="t"?d(u("x","tl"))*a+","+i(x)*a:o==="r"?i(e-x)*a+","+d(u("y","tr"))*a:o==="b"?i(e-u("x","br"))*a+","+d(f-x)*a:d(x)*a+","+i(f-u("y","bl"))*a;
o=o==="t"?i(e-u("x","tr"))*a+","+i(x)*a:o==="r"?i(e-x)*a+","+i(f-u("y","br"))*a:o==="b"?d(u("x","bl"))*a+","+d(f-x)*a:d(x)*a+","+d(u("y","tl"))*a;
return q?(y?"m"+o:"")+"l"+A:(y?"m"+A:"")+"l"+o};b=function(o,x,q,y,A,D){var B=o==="l"||o==="r",C=s[o],z,F;
if(C>0&&n[o]!=="none"&&p[o].la()>0){z=s[B?o:x];x=s[B?x:o];F=s[B?o:q];q=s[B?q:o];if(n[o]==="dashed"||n[o]==="dotted"){j.push({path:w(y,z,x,D+45,0,1)+w(y,0,0,D,1,0),fill:p[o]});
j.push({path:r(o,C/2,0,1),stroke:n[o],mb:C,color:p[o]});j.push({path:w(A,F,q,D,0,1)+w(A,0,0,D-45,1,0),fill:p[o]})
}else{j.push({path:w(y,z,x,D+45,0,1)+r(o,C,0,0)+w(A,F,q,D,0,0)+(n[o]==="double"&&C>2?w(A,F-d(F/3),q-d(q/3),D-45,1,0)+r(o,i(C/3*2),1,0)+w(y,z-d(z/3),x-d(x/3),D,1,0)+"x "+w(y,d(z/3),d(x/3),D+45,0,1)+r(o,d(C/3),1,0)+w(A,d(F/3),d(q/3),D,0,0):"")+w(A,0,0,D-45,1,0)+r(o,0,1,0)+w(y,0,0,D,1,0),fill:p[o]})
}}};b("t","l","r","tl","tr",90);b("r","t","b","tr","br",0);b("b","r","l","br","bl",-90);b("l","b","t","bl","tl",-180)
}}return j},n:function(){g.B.n.call(this);this.e.runtimeStyle.borderColor=""}});g.Ub=g.B.Z({C:5,Oc:["t","tr","r","br","b","bl","l","tl","c"],R:function(){return this.j.N.D()
},g:function(){return this.j.N.g()},X:function(){this.w();var a=this.j.N.m(),b=this.q.o(),c=this.e,e=this.Gb;
g.p.Pb(a.src,function(f){function h(w,r,o,x,q){w=e[w].style;var y=Math.max;w.width=y(r,0);w.height=y(o,0);
w.left=x;w.top=q}function j(w,r,o){for(var x=0,q=w.length;x<q;x++){e[w[x]].imagedata[r]=o}}var d=b.i,i=b.f,k=a.width,m=k.T.a(c),l=k.S.a(c),t=k.L.a(c);
k=k.Q.a(c);var n=a.slice,p=n.T.a(c),s=n.S.a(c),u=n.L.a(c);n=n.Q.a(c);h("tl",k,m,0,0);h("t",d-k-l,m,k,0);
h("tr",l,m,d-l,0);h("r",l,i-m-t,d-l,m);h("br",l,t,d-l,i-t);h("b",d-k-l,t,k,i-t);h("bl",k,t,0,i-t);h("l",k,i-m-t,0,m);
h("c",d-k-l,i-m-t,k,m);j(["tl","t","tr"],"cropBottom",(f.f-p)/f.f);j(["tl","l","bl"],"cropRight",(f.i-n)/f.i);
j(["bl","b","br"],"cropTop",(f.f-u)/f.f);j(["tr","r","br"],"cropLeft",(f.i-s)/f.i);if(a.repeat.kb==="stretch"){j(["l","r","c"],"cropTop",p/f.f);
j(["l","r","c"],"cropBottom",u/f.f)}if(a.repeat.f==="stretch"){j(["t","b","c"],"cropLeft",n/f.i);j(["t","b","c"],"cropRight",s/f.i)
}e.c.style.display=a.fill?"":"none"},this)},w:function(){var a=this.parent.ra(this.C),b,c,e,f=this.Oc,h=f.length;
if(!a){a=doc.createElement("border-image");b=a.style;b.position="absolute";this.Gb={};for(e=0;e<h;e++){c=this.Gb[f[e]]=g.p.Ga("rect");
c.appendChild(g.p.Ga("imagedata"));b=c.style;b.behavior="url(#default#VML)";b.position="absolute";b.top=b.left=0;
c.imagedata.src=this.j.N.m().src;c.stroked=false;c.filled=false;a.appendChild(c)}this.parent.Wa(this.C,a)
}return a}});g.Yb=g.B.Z({C:1,Fa:"outset-box-shadow",R:function(){var a=this.j;return a.W.D()||a.v.D()
},g:function(){var a=this.j.W;return a.g()&&a.m().ua[0]},X:function(){function a(z,F,H,K,J,v,E){z=b.sa("shadow"+z+F,"fill",e,j-z);
F=z.fill;z.coordsize=m*2+","+l*2;z.coordorigin="1,1";z.stroked=false;z.filled=true;F.color=J.O(c);if(v){F.type="gradienttitle";
F.color2=F.color;F.opacity=0}z.path=E;u=z.style;u.left=H;u.top=K;u.width=m;u.height=l;return z}var b=this,c=this.e,e=this.w(),f=this.j,h=f.W.m().ua;
f=f.v.m();var j=h.length,d=j,i,k=this.q.o(),m=k.i,l=k.f;k=g.J===8?1:0;for(var t=["tl","tr","br","bl"],n,p,s,u,w,r,o,x,q,y,A,D,B,C;
d--;){p=h[d];w=p.fd.a(c);r=p.gd.a(c);i=p.Xc.a(c);o=p.blur.a(c);p=p.color;x=-i-o;if(!f&&o){f=g.Oa.Qb}x=this.oa({T:x,S:x,L:x,Q:x},2,f);
if(o){q=(i+o)*2+m;y=(i+o)*2+l;A=o*2/q;D=o*2/y;if(o-i>m/2||o-i>l/2){for(i=4;i--;){n=t[i];B=n.charAt(0)==="b";
C=n.charAt(1)==="r";n=a(d,n,w,r,p,o,x);s=n.fill;s.focusposition=(C?1-A:A)+","+(B?1-D:D);s.focussize="0,0";
n.style.clip="rect("+((B?y/2:0)+k)+"px,"+(C?q:q/2)+"px,"+(B?y:y/2)+"px,"+((C?q/2:0)+k)+"px)"}}else{n=a(d,"",w,r,p,o,x);
s=n.fill;s.focusposition=A+","+D;s.focussize=1-A*2+","+(1-D*2)}}else{n=a(d,"",w,r,p,o,x);w=p.la();if(w<1){n.fill.opacity=w
}}}}});g.bc=g.B.Z({C:6,Fa:"imgEl",R:function(){var a=this.j;return this.e.src!==this.hc||a.v.D()},g:function(){var a=this.j;
return a.v.g()||a.H.Eb()},X:function(){this.hc=j;this.Gc();var a=this.sa("img","fill",this.w()),b=a.fill,c=this.q.o(),e=c.i;
c=c.f;var f=this.j.I.m();f=f&&f.nb;var h=this.e,j=h.src,d=Math.round;a.stroked=false;b.type="frame";b.src=j;
b.position=(e?0.5/e:0)+","+(c?0.5/c:0);a.coordsize=e*2+","+c*2;a.coordorigin="1,1";a.path=this.oa(f?{T:d(f.t.a(h)),S:d(f.r.a(h)),L:d(f.b.a(h)),Q:d(f.l.a(h))}:0,2);
a=a.style;a.width=e;a.height=c},Gc:function(){this.e.runtimeStyle.filter="alpha(opacity=0)"},n:function(){g.B.n.call(this);
this.e.runtimeStyle.filter=""}});g.Qa=function(){function a(d){function i(){if(!z){var v,E,G=d.currentStyle,I=G.getAttribute(c)==="true";
J=G.getAttribute(e);J=g.Ab===8?J!=="false":J==="true";if(!C){C=1;d.runtimeStyle.zoom=1;G=d;for(var O=1;
G=G.previousSibling;){if(G.nodeType===1){O=0;break}}if(O){d.className+=" "+g.Pa+"first-child"}}y.Ja();
if(I&&(E=y.o())&&(v=doc.documentElement||doc.body)&&(E.y>v.clientHeight||E.x>v.clientWidth||E.y+E.f<0||E.x+E.i<0)){if(!H){H=1;
g.Ra.aa(i)}}else{z=1;H=C=0;g.Ra.Ma(i);A={H:new g.Tb(d),I:new g.Xb(d),N:new g.Vb(d),v:new g.Oa(d),W:new g.Zb(d),lb:new g.ec(d)};
D=[A.H,A.I,A.N,A.v,A.W,A.lb];v=new g.cc(d,y,A);E=[new g.Yb(d,y,A,v),new g.Sb(d,y,A,v),new g.Wb(d,y,A,v),new g.Ub(d,y,A,v)];
d.tagName==="IMG"&&E.push(new g.bc(d,y,A,v));v.oc=E;q=[v].concat(E);if(v=d.currentStyle.getAttribute(g.F+"watch-ancestors")){B=[];
v=parseInt(v,10);E=0;for(I=d.parentNode;I&&(v==="NaN"||E++<v);){B.push(I);I.attachEvent("onpropertychange",u);
I.attachEvent("onmouseenter",p);I.attachEvent("onmouseleave",s);I=I.parentNode}}if(J){g.ya.aa(m);g.ya.Tc()
}m(1)}if(!F){F=1;d.attachEvent("onmove",k);d.attachEvent("onresize",k);d.attachEvent("onpropertychange",l);
d.attachEvent("onmouseenter",p);d.attachEvent("onmouseleave",s);g.za.aa(k);g.G.aa(o)}y.La()}}function k(){y&&y.Ec()&&m()
}function m(v){if(!K){if(z){var E,G;w();if(v||y.Qc()){E=0;for(G=q.length;E<G;E++){q[E].jb()}}if(v||y.Vc()){E=0;
for(G=q.length;E<G;E++){q[E].Lb()}}r()}else{C||i()}}}function l(){var v,E,G;v=event;if(!K&&!(v&&v.propertyName in j)){if(z){w();
v=0;for(E=q.length;v<E;v++){G=q[v];G.eb||G.jb();G.R()&&G.Kb()}r()}else{C||i()}}}function t(){if(d){d.className+=f
}}function n(){if(d){d.className=d.className.replace(h,"")}}function p(){setTimeout(t,0)}function s(){setTimeout(n,0)
}function u(){var v=event.propertyName;if(v==="className"||v==="id"){l()}}function w(){y.Ja();for(var v=D.length;
v--;){D[v].Ja()}}function r(){for(var v=D.length;v--;){D[v].La()}y.La()}function o(){if(F){if(B){for(var v=0,E=B.length,G;
v<E;v++){G=B[v];G.detachEvent("onpropertychange",u);G.detachEvent("onmouseenter",p);G.detachEvent("onmouseleave",s)
}}d.detachEvent("onmove",m);d.detachEvent("onresize",m);d.detachEvent("onpropertychange",l);d.detachEvent("onmouseenter",p);
d.detachEvent("onmouseleave",s);g.G.Ma(o);F=0}}function x(){if(!K){var v,E;o();K=1;if(q){v=0;for(E=q.length;
v<E;v++){q[v].n()}}J&&g.ya.Ma(m);g.za.Ma(m);q=y=A=D=B=d=null}}var q,y=new M(d),A,D,B,C,z,F,H,K,J;this.Ic=i;
this.update=m;this.n=x;this.Ac=d}var b={},c=g.F+"lazy-init",e=g.F+"poll",f=" "+g.Pa+"hover",h=new RegExp("\\b"+g.Pa+"hover\\b","g"),j={background:1,bgColor:1,display:1};
a.Cc=function(d){var i=g.p.ta(d);return b[i]||(b[i]=new a(d))};a.n=function(d){d=g.p.ta(d);var i=b[d];
if(i){i.n();delete b[d]}};a.wc=function(){var d=[],i;if(b){for(var k in b){if(b.hasOwnProperty(k)){i=b[k];
d.push(i.Ac);i.n()}}b={}}return d};return a}();g.attach=function(a){g.Ab<9&&g.Qa.Cc(a).Ic()};g.detach=function(a){g.Qa.n(a)
}}})();!function($){var Tooltip=function(element,options){this.init("tooltip",element,options)};Tooltip.prototype={constructor:Tooltip,init:function(type,element,options){var eventIn,eventOut;
this.type=type;this.$element=$(element);this.options=this.getOptions(options);this.enabled=true;if(this.options.trigger!="manual"){eventIn=this.options.trigger=="hover"?"mouseenter":"focus";
eventOut=this.options.trigger=="hover"?"mouseleave":"blur";this.$element.on(eventIn,this.options.selector,$.proxy(this.enter,this));
this.$element.on(eventOut,this.options.selector,$.proxy(this.leave,this))}this.options.selector?(this._options=$.extend({},this.options,{trigger:"manual",selector:""})):this.fixTitle()
...
if(!self.options.delay||!self.options.delay.show){self.show()}else{self.hoverState="in";setTimeout(function(){if(self.hoverState=="in"){self.show()
}},self.options.delay.show)}},leave:function(e){var self=$(e.currentTarget)[this.type](this._options).data(this.type);
if(!self.options.delay||!self.options.delay.hide){self.hide()}else{self.hoverState="out";setTimeout(function(){if(self.hoverState=="out"){self.hide()
}},self.options.delay.hide)}},show:function(){var $tip,inside,pos,actualWidth,actualHeight,placement,tp;
if(this.hasContent()&&this.enabled){$tip=this.tip();this.setContent();if(this.options.animation){$tip.addClass("dp-fade")
}placement=typeof this.options.placement=="function"?this.options.placement.call(this,$tip[0],this.$element[0]):this.options.placement;
inside=/in/.test(placement);$tip.remove().css({top:0,left:0,display:"block"}).appendTo(inside?this.$element:$("#dp"));
pos=this.getPosition(inside);actualWidth=$tip[0].offsetWidth;actualHeight=$tip[0].offsetHeight;switch(inside?placement.split(" ")[1]:placement){case"bottom":tp={top:pos.top+pos.height,left:pos.left+pos.width/2-actualWidth/2};
break;case"top":tp={top:pos.top-actualHeight,left:pos.left+pos.width/2-actualWidth/2};break;case"left":tp={top:pos.top+pos.height/2-actualHeight/2,left:pos.left-actualWidth};
break;case"right":tp={top:pos.top+pos.height/2-actualHeight/2,left:pos.left+pos.width};break}var noArrow=false;
if(tp.top<0){tp.top=8;noArrow=true}if(tp.left<0){tp.left=8;noArrow=true}if(noArrow){$tip.find(".dp-arrow").hide()
}else{$tip.find(".dp-arrow").show()}$tip.css(tp).addClass("dp-"+placement).addClass("dp-in")}},setContent:function(){var $tip=this.tip();
$tip.find(".dp-tooltip-inner").html(this.getTitle());$tip.removeClass("dp-fade dp-in dp-top dp-bottom dp-left dp-right")
},hide:function(){var that=this,$tip=this.tip();$tip.removeClass("dp-in");function removeWithAnimation(){var timeout=setTimeout(function(){$tip.off($.support.transition.end).remove()
},500);$tip.one($.support.transition.end,function(){clearTimeout(timeout);$tip.remove()})}$.support.transition&&this.$tip.hasClass("dp-fade")?removeWithAnimation():$tip.remove()
...
}},hasContent:function(){return this.getTitle()},getPosition:function(inside){return $.extend({},(inside?{top:0,left:0}:this.$element.offset()),{width:this.$element[0].offsetWidth,height:this.$element[0].offsetHeight})
},getTitle:function(){var title,$e=this.$element,o=this.options;title=$e.attr("data-original-title")||(typeof o.title=="function"?o.title.call($e[0]):o.title);
title=(title||"").toString().replace(/(^\s*|\s*$)/,"");return title},tip:function(){return this.$tip=this.$tip||$(this.options.template)
...
},toggle:function(){this[this.tip().hasClass("dp-in")?"hide":"show"]()}};$.fn.tooltip=function(option){return this.each(function(){var $this=$(this),data=$this.data("tooltip"),options=typeof option=="object"&&option;
if(!data){$this.data("tooltip",(data=new Tooltip(this,options)))}if(typeof option=="string"){data[option]()
}})};$.fn.tooltip.Constructor=Tooltip;$.fn.tooltip.defaults={animation:true,delay:0,selector:false,placement:"top",trigger:"hover",title:"",template:'<div class="dp-tooltip"><div class="dp-tooltip-arrow"></div><div class="dp-tooltip-inner"></div></div>'}
}(window.jQuery);!function($){var toggle='[data-toggle="dp-dropdown"]',Dropdown=function(element){var $el=$(element).on("click.dropdown.data-api",this.toggle);
$("html").on("click.dropdown.data-api",function(){$el.parent().removeClass("dp-open")})};Dropdown.prototype={constructor:Dropdown,toggle:function(e){var $this=$(this),selector=$this.attr("data-target"),$parent,isActive;
if(!selector){selector=$this.attr("href");selector=selector&&selector.replace(/.*(?=#[^\s]*$)/,"")}$parent=$(selector);
$parent.length||($parent=$this.parent());isActive=$parent.hasClass("dp-open");clearMenus();!isActive&&$parent.toggleClass("dp-open");
return false}};function clearMenus(){$(toggle).parent().removeClass("dp-open")}$.fn.dropdown=function(option){return this.each(function(){var $this=$(this),data=$this.data("dropdown");
if(!data){$this.data("dropdown",(data=new Dropdown(this)))}if(typeof option=="string"){data[option].call($this)
}})};$.fn.dropdown.Constructor=Dropdown;$(function(){$("html").on("click.dropdown.data-api",clearMenus);
$("body").on("click.dropdown.data-api",toggle,Dropdown.prototype.toggle)})}(window.jQuery);!function($){var Modal=function(content,options){this.options=options;
this.$element=$(content).delegate('[data-dismiss="dp-modal"]',"click.dismiss.modal",$.proxy(this.hide,this))
};Modal.prototype={constructor:Modal,toggle:function(){return this[!this.isShown?"show":"hide"]()},show:function(){var that=this;
if(this.isShown){return}$("body").addClass("dp-modal-open");this.isShown=true;this.$element.trigger("show");
escape.call(this);backdrop.call(this,function(){var transition=$.support.transition&&that.$element.hasClass("dp-fade");
!that.$element.parent().length&&that.$element.appendTo($("#dp"));that.$element.show();if(transition){that.$element[0].offsetWidth
}that.$element.addClass("dp-in");transition?that.$element.one($.support.transition.end,function(){that.$element.trigger("shown")
}):that.$element.trigger("shown")})},hide:function(e){e&&e.preventDefault();if(!this.isShown){return}var that=this;
this.isShown=false;$("body").removeClass("dp-modal-open");escape.call(this);this.$element.trigger("hide").removeClass("dp-in");
$.support.transition&&this.$element.hasClass("dp-fade")?hideWithTransition.call(this):hideModal.call(this)
}};function hideWithTransition(){var that=this,timeout=setTimeout(function(){that.$element.off($.support.transition.end);
hideModal.call(that)},500);this.$element.one($.support.transition.end,function(){clearTimeout(timeout);
hideModal.call(that)})}function hideModal(that){this.$element.hide().trigger("hidden");backdrop.call(this)
}function backdrop(callback){var that=this,animate=this.$element.hasClass("dp-fade")?"fade":"";if(this.isShown&&this.options.backdrop){var doAnimate=$.support.transition&&animate;
this.$backdrop=$('<div class="dp-modal-backdrop '+animate+'" />').appendTo($("#dp"));if(this.options.backdrop!="static"){this.$backdrop.click($.proxy(this.hide,this))
}if(doAnimate){this.$backdrop[0].offsetWidth}this.$backdrop.addClass("dp-in");doAnimate?this.$backdrop.one($.support.transition.end,callback):callback()
}else{if(!this.isShown&&this.$backdrop){this.$backdrop.removeClass("dp-in");$.support.transition&&this.$element.hasClass("dp-fade")?this.$backdrop.one($.support.transition.end,$.proxy(removeBackdrop,this)):removeBackdrop.call(this)
}else{if(callback){callback()}}}}function removeBackdrop(){this.$backdrop.remove();this.$backdrop=null
}function escape(){var that=this;if(this.isShown&&this.options.keyboard){$(document).on("keyup.dismiss.modal",function(e){e.which==27&&that.hide()
})}else{if(!this.isShown){$(document).off("keyup.dismiss.modal")}}}$.fn.modal=function(option){return this.each(function(){var $this=$(this),data=$this.data("modal"),options=$.extend({},$.fn.modal.defaults,$this.data(),typeof option=="object"&&option);
if(!data){$this.data("modal",(data=new Modal(this,options)))}if(typeof option=="string"){data[option]()
}else{if(options.show){data.show()}}})};$.fn.modal.defaults={backdrop:true,keyboard:true,show:true};$.fn.modal.Constructor=Modal;
$(function(){$("body").on("click.modal.data-api",'[data-toggle="dp-modal"]',function(e){var $this=$(this),href,$target=$($this.attr("data-target")||(href=$this.attr("href"))&&href.replace(/.*(?=#[^\s]+$)/,"")),option=$target.data("modal")?"toggle":$.extend({},$target.data(),$this.data());
e.preventDefault();$target.modal(option)})})}(window.jQuery);!function($){var Popover=function(element,options){this.init("popover",element,options)
};Popover.prototype=$.extend({},$.fn.tooltip.Constructor.prototype,{constructor:Popover,setContent:function(){var $tip=this.tip(),title=this.getTitle(),content=this.getContent();
$tip.find(".dp-popover-title")[$.type(title)=="object"?"append":"html"](title);$tip.find(".dp-popover-content > *")[$.type(content)=="object"?"append":"html"](content);
$tip.removeClass("dp-fade dp-top dp-bottom dp-left dp-right dp-in")},hasContent:function(){return this.getTitle()||this.getContent()
},getContent:function(){var content,$e=this.$element,o=this.options;content=$e.attr("data-content")||(typeof o.content=="function"?o.content.call($e[0]):o.content);
content=content.toString().replace(/(^\s*|\s*$)/,"");return content},tip:function(){if(!this.$tip){this.$tip=$(this.options.template)
}return this.$tip}});$.fn.popover=function(option){return this.each(function(){var $this=$(this),data=$this.data("popover"),options=typeof option=="object"&&option;
if(!data){$this.data("popover",(data=new Popover(this,options)))}if(typeof option=="string"){data[option]()
}})};$.fn.popover.Constructor=Popover;$.fn.popover.defaults=$.extend({},$.fn.tooltip.defaults,{placement:"right",content:"",template:'<div class="dp-popover"><div class="dp-arrow"></div><div class="dp-popover-inner"><h3 class="dp-popover-title"></h3><div class="dp-popover-content"><p></p></div></div></div>'})
}(window.jQuery);var Orb={};if(!window.console){window.console={}}["error","log","warn","info","debug","trace"].each(function(v){if(!window.console[v]){window.console[v]=function(){}
...
}}if(typeof DP_DEBUG=="undefined"||!DP_DEBUG){DP_DEBUG=false}var DP={console:{error:function(){},log:function(){},warn:function(){},info:function(){},debug:function(){},trace:function(){}},init:function(){if(typeof window.console!="undefined"){DP.console=window.console
...
DP.init();if(!Orb){var Orb={}}Orb.Class=function(properties){if(properties.DisableParentCall){function checkParentUse(obj){return false
...
if(info.mins<=15){fraction=""}else{if(info.mins<=30){fraction="¼"}else{if(info.mins<=45){fraction="½"
}else{if(info.mins<=60){fraction="¾"}}}}var phrase_num=info.hours;var phrase_hours=info.hours+"";if(fraction!==""){phrase_num+=1;
...
}});Orb.createNamespace("DeskPRO");DeskPRO.MessageBroker=new Orb.Class({Implements:[Orb.Util.Events],sendMessage:function(name,data){this.fireEvent(name,[data,name]);
...
}});Orb.createNamespace("DeskPRO");DeskPRO.BasicWindow=new Orb.Class({Implements:[Orb.Util.Options],initialize:function(options){$("html").addClass("window-active");
...
},getDebug:function(name){if(this.DEBUG[name]===undefined){return false}return this.DEBUG[name]}});Orb.createNamespace("DeskPRO.UI");
DeskPRO.UI.SimpleTabs=new Orb.Class({Implements:[Orb.Util.Options,Orb.Util.Events],initialize:function(options){this.options={triggerElements:".tab-trigger",activeClassname:"on",context:document,autoSelectFirst:true,effect:null};
...
},destroy:function(){}});(function($){$.fn.dpTwoSelect=function(options){$(this).each(function(){dpTwoSelectInit($(this),options)
});return this};function dpTwoSelectInit(el,options){var structure=[],has_optgroup=false,uid=0,blankOption,debugMessge,setValue,findValue,placeholder,parentElWrapper,parentEl,childElWrapper,childEl;
options=$.extend({debug:Dp_EnableDebug||false,renderPlaceholders:function(){var wrap=$('<div class="dp-two-select" />');
wrap.append('<div class="dp-parent"><select /></div>');wrap.append('<div class="dp-child"><select /></div>');
return wrap}},options||{});if(options.debug){debugMessge=function(){if(console.log&&console.log.apply){console.log.apply(console,arguments)
}}}else{debugMessge=function(){}}debugMessge("dpTwoSelect: %o %o",el,options);el.find("> optgroup, > option").each(function(){var opt=$(this),sub_structure,has_selected,id;
if(opt.is("optgroup")){sub_structure=[];has_optgroup=true;has_selected=false;opt.find("> option").each(function(){id="dpid_"+(uid++);
sub_structure.push({id:id,value:$(this).val(),label:$.trim($(this).text()),isSelected:$(this).is(":selected"),el:$(this)});
if($(this).is(":selected")){has_selected=true}});id="dpid_"+(uid++);structure.push({id:id,label:$.trim(opt.attr("label")),isGroup:true,value:id,isSelected:has_selected,subs:sub_structure,el:opt})
}else{if(opt.val()===""){blankOption=opt;return}id="dpid_"+(uid++);structure.push({id:id,value:opt.val(),label:$.trim(opt.text()),isSelected:opt.is(":selected"),el:opt})
}});debugMessge("dpTwoSelect structure: %o",structure);if(!has_optgroup){debugMessge("dpTwoSelect no optgroup, skipping");
return}findValue=function(val){var iParent=null,iChild=null;$.each(structure,function(iP,optInfo){if(optInfo.isGroup){if(optInfo.value==val){iParent=iP;
return false}$.each(optInfo.subs,function(iC,subOptInfo){if(subOptInfo.value==val){iParent=iP;iChild=iC;
return false}});if(iParent){return false}}else{if(optInfo.value==val){iParent=iP;iChild=null;return false
}}});if(iParent===null){return null}return[iParent,iChild]};setValue=function(val){var idx=findValue(val),parentOptInfo,subBlankOption,selectedVal,childOptInfo=false;
if(!idx){if(blankOption){placeholder.removeClass("dp-show-child");parentEl.find("option").first().prop("selected",true);
el.find('option[value=""], option:not([value])').first().prop("selected",true).trigger("change")}else{setValue(parentEl.find("option").first().data("dp-id"))
}return false}parentOptInfo=structure[idx[0]];if(idx&&idx[1]!==null&&typeof idx[1]!=="undefined"){childOptInfo=parentOptInfo.subs[idx[1]]
}parentEl.find("option."+parentOptInfo.id).prop("selected",true);if(parentOptInfo.isGroup){childEl.get(0).innerHTML="";
if(blankOption){subBlankOption=blankOption.clone();if(!childOptInfo){subBlankOption.prop("selected",true);
selectedVal=subBlankOption.val()}childEl.append(subBlankOption.clone())}$.each(parentOptInfo.subs,function(i,optInfo){var opt=$("<option />");
opt.addClass(optInfo.id);opt.attr("value",optInfo.value);opt.text(optInfo.label);if(childOptInfo&&optInfo.id==childOptInfo.id){opt.prop("selected",true);
selectedVal=opt.val()}childEl.append(opt)});childEl.hide().show();placeholder.addClass("dp-show-child")
}else{selectedVal=parentOptInfo.value;placeholder.removeClass("dp-show-child")}el.find('option[value="'+selectedVal+'"]').prop("selected",true).trigger("change")
};el.hide().addClass("dp-two-select").data("dp-two-select",this);placeholder=options.renderPlaceholders(el,options);
placeholder.insertAfter(el);parentElWrapper=placeholder.find(".dp-parent");parentEl=parentElWrapper.find("select");
childElWrapper=placeholder.find(".dp-child");childEl=childElWrapper.find("select");if(blankOption){parentEl.append(blankOption.clone())
}$.each(structure,function(i,optInfo){var opt=$("<option />");opt.addClass(optInfo.id).data("dp-id",optInfo.id);
if(optInfo.isGroup){opt.attr("value",optInfo.id);opt.text(optInfo.label)}else{opt.attr("value",optInfo.value);
opt.text(optInfo.label)}parentEl.append(opt)});parentEl.on("change",function(){var opt=$(this).find("option:selected");
var val=opt.val();setValue(val)});childEl.on("change",function(){var val=$(this).val();el.find('option[value="'+val+'"]').prop("selected",true).trigger("change")
});setValue(el.find("option:selected").val());this.setValue=function(val){setValue(val)};this.getOptions=function(){return options
};this.getPlaceholder=function(){return placeholder};this.getParentEl=function(){return parentEl};this.getChildEl=function(){return childEl
}}})(jQuery);Orb.createNamespace("DeskPRO.UI");DeskPRO.UI.Overlay=new Orb.Class({Implements:[Orb.Util.Options,Orb.Util.Events],initialize:function(options){this.objectId=null;
...
},initPage:function(){var self=this;var isIE=(navigator.appVersion.toLowerCase().indexOf("msie")!=-1);
if(isIE){$("html").addClass("browser-ie")}if(!isIE){$(document).bind("drop dragover",function(e){e.preventDefault()
...
$("body").removeClass("file-drag-over")},100)})}var calcHeights=function(){var h=$("#dp_sidebar").height();
$("#dp_content").css({"min-height":h})};if(!window.IS_ADMIN_CONTROLS){calcHeights()}if(this.PAGE){this.PAGE.initPage()
}this.elementHandlers={};this.initFeatures(document);this.initLanguageChoices();$("#dp_footer").css({"display":"block","opacity":1,"visibility":"visible"});
$(".dp-copy").css({"display":"block","opacity":1,"visibility":"visible"}).find("a").css({"display":"inline","opacity":1,"visibility":"visible"});
...
if($("html").attr("dir")=="rtl"&&$("#share-this-js").length){var interval;interval=setInterval(function(){var wrapper=$("#stwrapper");
if(wrapper.length){wrapper.css({left:"",right:"-999px"});clearInterval(interval)}},100)}window.setTimeout(function(){self.sessionPing()
},600000);$(".dp-its-a-trap").find("input").on("change keypress",function(){$(this).val("")}).val("")
},sessionPing:function(){var self=this;$.ajax({url:BASE_PATH+"dp.php/session-ping.json?i=u",cache:false,complete:function(){window.setTimeout(function(){self.sessionPing()
},600000)},success:function(data){if(data.request_token){window.DP_REQUEST_TOKEN=data.request_token}}})
},initFeatures:function(contextEl){var self=this;$(".with-handler[data-element-handler]",contextEl).each(function(){var el=$(this);
var className=el.data("element-handler");var classObj=Orb.getNamespacedObject(className);if(!classObj){DP.console.error("Unknown portal handler `%s` on element %o",className,this);
return}if(!el.attr("id")){el.attr("id",Orb.getUniqueId("portal_"))}var obj=new classObj({el:el});self.elementHandlers[el.attr("id")]=obj
});$("form.with-form-validator",contextEl).each(function(){var v=new DeskPRO.Form.FormValidator($(this));
$(this).data("form-validator-inst",v)});$("a.in-overlay").on("click",function(ev){ev.preventDefault();
var el=$(this);var url=el.attr("href");if(url.indexOf("?")!==-1){url+="&_partial"}else{url+="?_partial"
}var overlay=new DeskPRO.UI.Overlay({contentMethod:"ajax",contentAjax:{url:url},destroyOnClose:true});
overlay.open()});$(".timeago").timeago();$("input.datepicker, .datepicker input").datepicker({dateFormat:"yy-mm-dd"});
$(document).on("click",".dp-bound-faded",function(){var parent=$(this).parent();var link=$("a[href]",parent).first();
window.location=link.attr("href")});$(".dp-inplace-drop").each(function(){var sel=$(this).find("select").first();
var label=$(this).find(".dp-opt-label");var updateTitle=function(){var opt=sel.find("option:selected").first();
if(!opt[0]){opt=sel.find("option").first()}label.text(opt.text());if(sel.data("bind-to")){var bound=$(sel.data("bind-to"));
bound.text(opt.text())}};sel.on("change",function(){updateTitle()});updateTitle()});if(typeof $.prototype.popover!="undefined"){$(".dp-help-pop").popover({placement:function(tip,element){var $element=$(element);
var pos=$.extend({},$element.offset(),{width:element.offsetWidth,height:element.offsetHeight});var actualWidth=355;
var actualHeight=200;var boundTop=$(document).scrollTop();var boundLeft=$(document).scrollLeft();var boundRight=boundLeft+$(window).width();
var boundBottom=boundTop+$(window).height();var elementAbove={top:pos.top-actualHeight,left:pos.left+pos.width/2-actualWidth/2};
var elementBelow={top:pos.top+pos.height,left:pos.left+pos.width/2-actualWidth/2};var elementLeft={top:pos.top+pos.height/2-actualHeight/2,left:pos.left-actualWidth};
var elementRight={top:pos.top+pos.height/2-actualHeight/2,left:pos.left+pos.width};var above=isWithinBounds(elementAbove);
var below=isWithinBounds(elementBelow);var left=isWithinBounds(elementLeft);var right=isWithinBounds(elementRight);
function isWithinBounds(elementPosition){return boundTop<elementPosition.top&&boundLeft<elementPosition.left&&boundRight>(elementPosition.left+actualWidth)&&boundBottom>(elementPosition.top+actualHeight)
}var position;if(above){position="top"}else{if(below){position="bottom"}else{if(left){position="left"
}else{position="right"}}}if($("html").attr("dir")=="rtl"){setTimeout(function(){var $tip=$(tip);$tip.css({left:"",right:""});
if(position=="top"||position=="bottom"){$tip.css({right:$(window).width()-pos.left-pos.width/2-$tip.width()/2,left:""})
}else{if(position=="left"){$tip.css({right:$(window).width()-pos.left,left:""})}else{if(position=="right"){$tip.css({right:$(window).width()-pos.left-pos.width-$tip.width(),left:""})
}}}},0)}return position}})}},initLanguageChoices:function(){var select=$("#dp_lang_chooser_form").find("select");
if(!select.length){return}var choices={},selected;select.find("option").each(function(){var $this=$(this);
choices[$this.attr("value")]={name:$this.html(),val:$this.attr("value"),flag:$this.data("flag")};if($this.attr("selected")){selected=$this.attr("value")
}});if(!selected){selected=select.find("option:first").attr("value")}var getChoiceHtml=function(choice){if(choice.flag&&choice.flag.length){return'<img src="'+ASSETS_BASE_URL+"/images/flags/"+choice.flag+'" class="flag" alt="" /> '+choice.name
}else{return'<span class="flag"></span> '+choice.name}};var choiceHtml=$('<span class="language-choice" />'),innerHtml=$('<span class="country-name" />').html(getChoiceHtml(choices[selected]));
choiceHtml.append(innerHtml);choiceHtml.append($('<span class="drop" />'));var choiceMenu=$('<ul class="language-options-list" />').css({display:"none",position:"absolute"});
for(var i in choices){var li=$("<li />").html(getChoiceHtml(choices[i]));(function(choice){li.click(function(){select.val(choice.val);
select.closest("form").submit();choiceMenu.hide();innerHtml.html(getChoiceHtml(choice))})})(choices[i]);
choiceMenu.append(li)}$("#dp").append(choiceMenu);innerHtml.css({width:choiceMenu.outerWidth()-24});var updatePos=function(){if(!choiceMenu.hasClass("open")){return
}var pos=choiceHtml.offset();choiceMenu.css({top:pos.top+choiceHtml.outerHeight(true)-1,left:pos.left})
};$(window).on("resize",updatePos);choiceHtml.click(function(){choiceMenu.addClass("open");updatePos();
choiceMenu.show()});select.hide();select.after(choiceHtml);$(document).click(function(e){if(!choiceHtml.find(e.target).length&&!choiceMenu.find(e.target).length){choiceMenu.hide().removeClass("open")
}})},showAutoSignInOverlay:function(){var overlay=$("#auto-sign-in-overlay"),overlayWidth=overlay.width();
overlay.css({left:Math.floor(($(document.body).width()-overlayWidth)/2)}).show()},getHandler:function(id){return this.handlers[id]
},hasHandler:function(id){return !!this.handlers[id]},setPageHandler:function(page){this.PAGE=page},getPageHandler:function(){return this.PAGE
}});function Orb_Util_TimeAgo_getPhraseFor(type,num,ago){var phrasepre="time";if(ago){phrasepre="time-ago"
}if(type=="min"){type="minute"}else{if(type=="mins"){type="minutes"}else{if(type=="sec"){type="second"
}else{if(type=="secs"){type="seconds"}}}}var phrasename="user.time."+phrasepre+"_x_"+type;if(num==1){var phrasename="user.time."+phrasepre+"_1_"+type
}if(type=="sec"&&num<=0){var phrasename="user.time."+phrasepre+"_less_second"}if(!window.DESKPRO_LANG||!DESKPRO_LANG[phrasename]){if(window.console&&window.console.warn){console.warn("Missing phrase %s",phrasename)
}return"timeago_missing_phrase: "+phrasename}return(DESKPRO_LANG[phrasename]||"").replace(/\{0\}/g,num)
...
this.timeout=null}this.options=null;this.lastTime=null}});Orb.createNamespace("DeskPRO");DeskPRO.Translate=new Orb.Class({initialize:function(loader){this.phrases={};
this.loader=loader||function(id){return"["+id+"]"};if(window.DESKPRO_LANG){this.phrases=window.DESKPRO_LANG
}},getPhraseText:function(phrase_name){if(typeof this.phrases[phrase_name]=="undefined"){this.phrases[phrase_name]=this.loader(phrase_name)
...
}}return false}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.ElementHandlerAbstract=new Orb.Class({Implements:[Orb.Util.Options],initialize:function(options){this.options={};
if(options){this.setOptions(options)}this.el=null;if(this.options.el){this.el=$(this.options.el)}this.init()
},init:function(){}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.MoreLoader=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){this.listEl=$(".content-list:first ul:first",this.el);
this.moreWrap=$(".content-more:first",this.el);this.moreBtn=$("button:first, a.button:first",this.el).first();
this.loadUrl=this.el.data("load-url");this.currentPage=1;var self=this;this.moreBtn.on("click",function(ev){ev.preventDefault();
ev.stopPropagation();self.loadNextPage()})},loadNextPage:function(){if(this.moreWrap.is(".loading")){return
}this.moreWrap.addClass("loading");var url=this.loadUrl.replace("{page}",this.currentPage+1);if(url.indexOf("?")===-1){url+="?_partial=more"
}else{url+="&_partial=more"}$.ajax({url:url,dataType:"html",context:this,success:function(html){this.insertNewItems(html.trim())
}})},insertNewItems:function(html){if(html.length){var els=$(html)}else{var els=null}this.currentPage++;
this.moreWrap.removeClass("loading");if(!els||!els.length||!els.last().is(".has-more")){this.moreWrap.remove()
}this.listEl.append(els)}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.LoginBox=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){this.loginBox=$("#dp_login_box");
this.loginSection=$(".dp-login-section",this.el);this.resetSection=$(".dp-reset-section",this.el);this._initResetSection()
},_initResetSection:function(){$(".forgot",this.el).on("click",(function(ev){ev.preventDefault();this.showReset()
}).bind(this));$(".back",this.resetSection).on("click",(function(ev){this.hideReset()}).bind(this));$(".dp-do-send",this.resetSection).on("click",(function(ev){ev.preventDefault();
this.sendReset()}).bind(this))},sendReset:function(){this.resetSection.addClass("loading");$.ajax({url:BASE_URL+"login/reset-password/send.json",type:"POST",data:{email:$("#dp_login_email").val()},dataType:"json",context:this,success:function(){this.resetSection.removeClass("loading");
var descEl=$(".dp-reset-desc",this.resetSection);var sentEl=$(".dp-reset-sent",this.resetSection);descEl.slideUp("fast",function(){sentEl.slideDown()
})}})},showReset:function(){this.loginSection.slideUp("fast",(function(){this.resetSection.slideDown("fast")
}).bind(this))},hideReset:function(quick){if(quick){this.resetSection.hide();this.loginSection.show();
$(".dp-reset-desc",this.resetSection).show();$(".dp-reset-sent",this.resetSection).hide()}else{this.resetSection.slideUp("fast",(function(){this.loginSection.slideDown("fast");
$(".dp-reset-desc",this.resetSection).show();$(".dp-reset-sent",this.resetSection).hide()}).bind(this))
}}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.NewTicket=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){this.titleTxt=$("#newticket_ticket_subject");
this.messageTxt=$("#newticket_ticket_message");this.ticketForm=$("#dp_newticket_form");$("#dp_newticket_form").find("select").not(".no-dp-select").dpTwoSelect();
this._initSuggestionsBox();this._initFields();this._initLoginForm(this.el);this._initPreticketStatus()
},_initSuggestionsBox:function(){if(this.el.data("suggestions-url")){this.inlineSuggestions=new DeskPRO.User.InlineSuggestions({elementWrapper:this.el,titleText:this.titleTxt,contentText:this.messageTxt,onResolved:this.setTicketSolvedAjax.bind(this),onResolvedRedirect:this.setTicketSolvedRedirect.bind(this),onNotResolved:this.setTicketUnsolvedAjax.bind(this)})
}},setTicketSolvedAjax:function(content_type,content_id){this.setTicketSolvedStatusAjax(content_type,content_id,false)
},setTicketUnsolvedAjax:function(content_type,content_id){this.setTicketSolvedStatusAjax(content_type,content_id,true)
},setTicketSolvedStatusAjax:function(content_type,content_id,setUnsolved){var preticket_id=$("#dp_newticket_preticket_status_id").val();
if(preticket_id>0){url=BASE_URL+"tickets/new/content-solved-save.json?"+"preticket_status_id="+escape(preticket_id)+"&"+"content_type="+escape(content_type)+"&"+"content_id="+escape(content_id);
if(setUnsolved){url+="&add_unsolved=1"}$.ajax({url:url,type:"GET"})}},setTicketSolvedRedirect:function(url,content_type,content_id){var preticket_id=$("#dp_newticket_preticket_status_id").val();
if(preticket_id>0){url=BASE_URL+"tickets/new/content-solved-redirect?"+"preticket_status_id="+escape(preticket_id)+"&"+"content_type="+escape(content_type)+"&"+"content_id="+escape(content_id)+"&"+"url="+escape(url)
}window.location=url},_initFields:function(){this.depSelect=$("select.department_id, input.department_id",this.el).first();
this.departmentId=-1;var self=this;this.depSelect.on("change",function(){self.handleDepChange()});this.depSelect.data("original-name",this.depSelect.attr("name"));
$("select.category_id, select.priority_id, select.product_id, .dp-two-select select").on("change",function(){self.runChecks()
});this.handleDepChange()},handleDepChange:function(){this.setDepartment(this.depSelect.val())},setDepartment:function(department_id){if(department_id==this.departmentId){return
}this.clearAll();this.departmentId=department_id;var activeDepId=this.departmentId;if(!window.DESKPRO_TICKET_DISPLAY){return
}if(!activeDepId||!window.DESKPRO_TICKET_DISPLAY[activeDepId]){activeDepId=0}var depItems=window.DESKPRO_TICKET_DISPLAY[activeDepId];
this.depItems=depItems;this.depItemsWithChecked=false;DP.console.log("depItems %o",depItems);$(".ticket-display-field").hide();
Array.each(depItems,function(item){var itemId=this.getItemId(item);var itemEl=$("."+itemId).closest(".ticket-display-field");
itemEl.detach().appendTo("#fields_container");if(!item.check){itemEl.show()}else{itemEl.addClass("with-criteria");
this.depItemsWithChecked=true}},this);this.runChecksRecursionCount=0;if(this.depItemsWithChecked){this.runChecks()
}},runChecks:function(){if(this.runChecksRecursionCount>30){console.error("runChecks running too many times: %o",this.depItems);
return}var self=this;var changed=false;$(".with-criteria").each(function(){var el=$(this);var item=self.findItemForEl(el);
if(!item){return}if(item.check(ticketReader)){if(!el.is(":visible")){changed=true}el.show()}else{if(el.is(":visible")){changed=true
}el.hide()}});if(changed){this.runChecksRecursionCount++;this.runChecks();this.runChecksRecursionCount--
}},findItemForEl:function(el){var fieldId=el.data("field-id");var theitem=null;Array.each(this.depItems,function(item){if(item.id==fieldId){theitem=item;
return false}});return theitem},clearAll:function(){$(".ticket-display-field").hide().removeClass("field-enabled with-criteria")
},getItemId:function(item){var itemId=item.field_type;if(item.field_id){itemId+="_"+item.field_id}return itemId
},_initLoginForm:function(context){this.inlineLogin=new DeskPRO.User.InlineLoginForm({context:this.el})
},_initPreticketStatus:function(){$("input, select",this.ticketForm).on("change",(function(){this.updatePreticketStatus()
}).bind(this))},updatePreticketStatus:function(){var formData=this.ticketForm.serializeArray();$.ajax({url:BASE_URL+"tickets/new/save-status",type:"POST",data:formData,dataType:"json",success:function(data){$("#dp_newticket_preticket_status_id").val(data.preticket_status_id)
}})}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.FormUploadHandler=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){var self=this;
var dropZone=this.el;if(this.el.data("drop-document")=="1"){dropZone=$(document)}var options={url:this.el.data("upload-to"),dropZone:dropZone,autoUpload:true,formData:{security_token:this.el.data("security-token")},done:function(e,data){var that=$(this).data("fileupload"),template,preview;
if(!that){return}if(data.context){data.context.each(function(index){var file=($.isArray(data.result)&&data.result[index])||{error:"emptyResult"};
if(file.error&&that._adjustMaxNumberOfFiles){that._adjustMaxNumberOfFiles(1)}that._transition($(this)).done(function(){var node=$(this);
template=that._renderDownload([file]).css("height",node.height()).replaceAll(node);that._forceReflow(template);
that._transition(template).done(function(){data.context=$(this);that._trigger("completed",e,data)})})
})}else{template=that._renderDownload(data.result).appendTo(that.options.filesContainer);that._forceReflow(template);
that._transition(template).done(function(){data.context=$(this);that._trigger("completed",e,data)})}if(that.options.filesContainer){if(!that.options.filesContainer.find(".uploading")[0]){self.el.trigger("dp_upload_all_done")
}}}};this._handleOptions(options);this.el.fileupload(options);$(".dp-fallback",this.el).remove();$(".dp-good-upload",this.el).show()
},_handleOptions:function(options){var el=this.el;if(!options.namespace){options.namespace=Orb.uuid()
}if(!options.dropZone){options.dropZone=$(el)}if(typeof options.autoUpload=="undefined"){options.autoUpload=true
}if(options.uploadTemplate){var setel=options.uploadTemplate}else{var setel=$(".template-upload",el)}if(!setel.attr("id")){var id=Orb.getUniqueId("up");
...
}if(!setel.attr("id")){var id=Orb.getUniqueId("up");setel.attr("id",id)}else{var id=setel.attr("id")}delete (options.downloadTemplate);
options.downloadTemplateId=id;if(!options.filesContainer){options.filesContainer=$(el).find(".files")
}options.start=function(){$(el).find(".error").remove()};$(el).on("click",".remove-attach-trigger",function(ev){if($(this).hasClass("delete")){return
}ev.preventDefault();var el=$(this).closest("li");el.slideUp("fast",function(){el.remove()})})}});Orb.createNamespace("DeskPRO.User.ElementHandler");
DeskPRO.User.ElementHandler.TicketList=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){$("a.close-ticket-trigger").on("click",function(ev){var text="Once a ticket is closed, our agents will not reply to it anymore. Are you sure you want to close this ticket?";
if(!confirm(text)){ev.preventDefault()}})}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.TicketView=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){$(".feedback-link",this.el).each(function(){var el=$(this);
$("a",this).on("click",function(ev){ev.preventDefault();window.open($(this).attr("href"),"feedback","status=no,toolbar=no,location=no,menubar=no,resizable=no,scrollbars=yes,height=310,width=720")
}).on("mouseover",function(){if($(this).is(".helpful")){el.addClass("rating-helpful").removeClass("rating-not-helpful")
}else{el.removeClass("rating-helpful").addClass("rating-not-helpful")}}).on("mouseout",function(){el.removeClass("rating-helpful").removeClass("rating-not-helpful")
})})}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.InlineEmailManage=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){var self=this;
this.emailField=$(".dp-email-field",this.el);this.newEmailField=$(".dp_inline_email_new",this.el);this.emailList=$(".dp-email-manage-list",this.el);
this.controlsEl=$(".dp-email-manage-controls",this.el);$('input[name="dp_inline_email_choice"]',this.controlsEl).on("click",function(){var val=$(this).val();
if(val=="NEW"){self.setNewMode()}else{self.emailField.val(val)}});this.emailError=$(".error-message",this.el);
this.newEmailField.on("keypress",function(ev){if(ev.keyCode==13){ev.preventDefault();ev.stopPropagation()
}});this.newEmailField.on("keyup",function(ev){if(self.mode=="new"){self.emailField.val(self.newEmailField.val())
}self.newEmailField.removeClass("error")});this.mode="normal";if($('input[name="dp_inline_email_choice_radio"]:checked').val()=="NEW"){this.setNewMode()
}$('input[name="dp_inline_email_choice_radio"]').not('[value="NEW"]').on("click",function(){self.setNormalMode();
self.emailField.val($(this).val())});$('input[name="dp_inline_email_choice_radio"][value="NEW"]').on("click",function(){self.emailField.val(self.newEmailField.val());
self.setNewMode()});var changeEmail=$(".change-email",this.el).on("click",function(ev){ev.preventDefault();
self.controlsEl.slideDown("fast");changeEmail.hide();changeEmailClose.show()});var changeEmailClose=$(".change-email-close",this.el).on("click",function(ev){ev.preventDefault();
if($('input[name="dp_inline_email_choice_radio"][value="NEW"]').is(":checked")){var email=self.newEmailField.val();
if(!email||email.indexOf("@")===-1){self.newEmailField.addClass("error").focus();return}}self.controlsEl.slideUp("fast");
changeEmail.show();changeEmailClose.hide()})},setNormalMode:function(){this.mode="normal"},setNewMode:function(){this.mode="new";
this.emailField.val(this.newEmailField.val())}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.ElementHandler.CommentFormLogin=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){var self=this;
window.DP_LOGIN_NOTIFY=function(){$("#comments_form_login_area").empty();$.ajax({url:BASE_URL+"_misc/comment-form-login-partial",dataType:"html",success:function(html){$("#comments_form_login_area").html(html)
}})};this.el.find(".auth-popup").click(function(ev){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();
window.open($(this).attr("href"),"dpauth","width=600,height=400,location=0,menubar=0,scrollbars=0,status=0,toolbar=0,resizable=0")
})}});Orb.createNamespace("DeskPRO.User.ElementHandler.Helper");DeskPRO.User.ElementHandler.FeedbackAgreeBtn=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){var self=this;
if(this.el.hasClass("dp-feedback-closed")){return}this.el.find(".dp-feedback-btn").on("click",function(){self.voteOnElement(self.el)
})},voteOnElement:function(el){var rating;if(el.is(".dp-voted")){rating=0}else{rating=1}var parentRow=el.closest(".dp-feedback-row");
var old_num=parseInt($("em",el).first().text());if(rating){el.addClass("dp-voted");parentRow.addClass("dp-voted-row");
$("em",el).first().text(old_num+1)}else{el.removeClass("dp-voted");parentRow.removeClass("dp-voted-row");
$("em",el).first().text(old_num-1)}$.ajax({url:el.data("vote-url"),data:{rating:rating},dataType:"json",context:this,type:"POST",success:function(data){if(data.voted){el.addClass("dp-voted");
parentRow.addClass("dp-voted-row")}else{el.removeClass("dp-voted");parentRow.removeClass("dp-voted-row")
}$("em",el).first().text(data.total_rating)}})}});Orb.createNamespace("DeskPRO.User.ElementHandler");
DeskPRO.User.ElementHandler.OmniSearch=new Orb.Class({Extends:DeskPRO.User.ElementHandler.ElementHandlerAbstract,init:function(){var self=this;
this.backdrop=$('<div class="dp-backdrop" />').hide().appendTo("#dp");this.assistEl=$("#dp_search_assist");
this.searchboxEl=$("#dp_search");this.resultsEl=$("div.results",this.assistEl);$("#dp_search_assist").on("click",function(){$("#dp_omnisearch").submit()
});this.searchboxEl.on("focus",this.activateAssist.bind(this));this.backdrop.on("click",function(ev){ev.stopPropagation();
self.deactivateAssist()});this.isActivated=false;this.searchTimer=new DeskPRO.IntervalCaller({touchResets:true,touchRequired:true,resetTimeForce:1500,timeout:750,autostart:true,callback:this.updateResults.bind(this)});
this.searchboxEl.on("keyup",function(ev){if(!$(this).val().trim().length){self.close()}else{self.activateAssist();
if(ev.keyCode=="32"){self.searchTimer.exec(true)}else{self.searchTimer.touch()}}});this.searchboxEl.on("focus",function(){self.activateAssist()
});this.lastTerms=null;$(".foot a",this.assistEl).on("click",function(ev){var el=$(this);if(el.is(".no-omni-trigger")||el.closest("li").is(".no-omni-trigger")){return
}ev.preventDefault();ev.stopPropagation();var url=el.attr("href");url=Orb.appendQueryData(url,"q",self.searchboxEl.val().trim());
window.location=url})},activateAssist:function(){var wasActive=this.isActivated;this.isActivated=true;
this.updatePosition();if(!this.searchboxEl.val().trim().length){this.deactivateAssist()}else{this.open();
if(!wasActive){this.searchTimer.execNow()}}},open:function(){this.assistEl.show();this.backdrop.show()
},deactivateAssist:function(){this.isActivated=false;this.close()},close:function(){this.assistEl.hide();
this.backdrop.hide()},updatePosition:function(){var pos=this.searchboxEl.offset();var w=this.searchboxEl.outerWidth();
var h=this.searchboxEl.outerHeight();this.assistEl.css({top:pos.top+h+1,left:pos.left,width:w-1})},updateResults:function(){if(!this.isActivated){return
}var terms=this.searchboxEl.val().trim();if(terms==this.lastTerms||terms===""){return}this.lastTerms=terms;
this.searchboxEl.addClass("loading");$.ajax({url:BASE_URL+"search/omnisearch/"+encodeURI(terms),dataType:"html",context:this,complete:function(){this.searchboxEl.removeClass("loading")
},success:function(html){if(html.indexOf("<body")!==-1||html.indexOf("<html")!==-1){return}var wrap=$(html);
this.resultsEl.empty();if(!$("li",wrap).length){}else{this.resultsEl.append(wrap);if(this.resultsEl.find("li").length>=10){$("#dp_search_assist").find(".dp-more-link").show()
}else{$("#dp_search_assist").find(".dp-more-link").hide()}this.open()}}})}});Orb.createNamespace("DeskPRO.User.ElementHandler");
DeskPRO.User.SuggestedContentOverlay=new Orb.Class({Implements:[Orb.Util.Options,Orb.Util.Events],initialize:function(options){this.options={template:null,url:null,pageUrl:null,contentType:null,contentId:null,destroyOnClose:true,maxHeight:700,openNear:null};
this.setOptions(options);this.overlayEl=null;this.backdropEl=null;this.runningAjax=false},_initOverlay:function(){if(this._hasInit){return
}this._hasInit=true;var self=this;this.overlayEl=$(this.options.template).hide().appendTo("#dp");this.controlsWrap=$(".dp-controls",this.overlayEl).hide();
this.backdropEl=$('<div class="dp-backdrop dp-faded" />').appendTo("#dp");this.backdropEl.on("click",function(el){self.close()
});$(".dp-close-btn",this.overlayEl).on("click",function(ev){ev.preventDefault();self.close()});this.runningAjax=$.ajax({url:this.options.url,type:"GET",context:this,error:function(){this.close()
},success:function(html){this.controlsWrap.show();$(".dp-content-holder",this.overlayEl).empty().html(html);
$(".dp-section-toggle",this.controlsWrap).on("click",function(ev){ev.preventDefault();var toggleSel=$(this).data("toggle-section");
$(".dp-control-section",self.controlsWrap).fadeOut("fast",function(){window.setTimeout(function(){$(toggleSel,self.controlsWrap).fadeIn()
},150)})});$(".dp-toggle-sel",this.overlayEl).on("click",function(ev){ev.preventDefault();var el=$($(this).data("toggle-sel"),self.overlayEl);
if($(this).is(".open")){$(this).removeClass("open");el.slideUp()}else{if($(this).data("toggle-self")){$(this).slideUp("fast",function(){el.slideDown()
}).addClass("open")}else{el.slideDown();$(this).addClass("open")}}});if(self.options.pageUrl){$(".dp-open-full",this.overlayEl).on("click",function(ev){ev.preventDefault();
window.open(self.options.pageUrl);self.close()});$(".dp-open-full a",this.overlayEl).attr("href",self.options.pageUrl)
}this.fireEvent("init",[this.overlayEl,this.controlsEl,this])}})},open:function(){this._initOverlay();
var pos={top:40,left:100,width:null};this.fireEvent("preOpen",[pos,this]);if(pos.cancel){return}var h=this.overlayEl.height();
var w=this.overlayEl.width();var winH=$(window).height();var winW=$(window).width();h=winH*0.7;w=winW*0.6;
if(h<250){h=250}if(w<400){w=400}if(h>this.options.maxHeight){h=this.options.maxHeight}this.overlayEl.css("height",h);
this.overlayEl.css("width",w);this.overlayEl.css({top:(winH/2)-(h/2),left:(winW/2)-(w/2)});this.overlayEl.fadeIn("fast").addClass("open");
this.backdropEl.show()},close:function(){if(!this._hasInit){return}if(!this.overlayEl.is(".open")){return
}this.overlayEl.fadeOut("fast",(function(){if(this.options.destroyOnClose){this.destroy()}}).bind(this));
this.backdropEl.hide()},destroy:function(){if(this._hasInit){return}this.overlayEl.remove();this.backdropEl.remove();
this.overlayEl=null;this.backdropEl=null;if(this.runningAjax){this.runningAjax.abort();this.runningAjax=null
}}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.InlineSuggestions=new Orb.Class({Implements:[Orb.Util.Options,Orb.Util.Events],initialize:function(options){this.options={elementWrapper:null,titleText:"#__dp_nomatch",contentText:"#__dp_nomatch"};
this.setOptions(options);this.el=$(this.options.elementWrapper);this.titleTxt=$(this.options.titleText);
this.messageTxt=$(this.options.contentText);this._initSuggestionsBox()},_initSuggestionsBox:function(){this.suggestionsBox=$(".dp-related-search",this.el);
this.resultsEl=$(".results",this.suggestionsBox);this.moreLink=$(".more-link",this.suggestionsBox);this.lastSuggestions=null;
this.lastString=null;this.notAnsweredResults=[];this.hasStartedSearch=false;this.moreLink.on("click",(function(ev){this.moreLink.hide();
$("li",this.resultsEl).show()}).bind(this));this.suggestionsUrl=this.el.data("suggestions-url");this.sugTitleTimer=null;
this.sugMessageTimer=null;this.titleTxt.on("keypress",(function(){if(!this.hasStartedSearch){return}if(this.sugTitleTimer){return
}this.sugTitleTimer=this.updateSuggestions.delay(400,this)}).bind(this));this.titleTxt.on("blur",(function(){this.hasStartedSearch=true;
this.updateSuggestions()}).bind(this));this.messageTxt.on("keypress",(function(){if(!this.hasStartedSearch){return
}if(this.sugMessageTimer){return}this.sugMessageTimer=this.updateSuggestions.delay(1200,this)}).bind(this))
},updateSuggestions:function(){if(this.sugTitleTimer){window.clearTimeout(this.sugTitleTimer);this.sugTitleTimer=null
}if(this.sugMessageTimer){window.clearTimeout(this.sugMessageTimer);this.sugMessageTimer=null}var content=(this.titleTxt.val().trim()+" "+this.messageTxt.val().trim()).trim();
if(this.lastSearchString&&this.lastSearchString==content){return}this.lastSearchString=content;if(!content.length){this.suggestionsBox.hide();
return}if(this.doSuggestResend){return}if(this.isSuggestActive){this.doSuggestResend=true;return}this.isSuggestActive=true;
$.ajax({url:this.suggestionsUrl,dataType:"html",data:{"content":content},context:this,success:function(html){this.isSuggestActive=false;
if(this.doSuggestResend){this.doSuggestResend=false;this.updateSuggestions()}if(this.lastSuggestions&&this.lastSuggestions==html){return
}this.lastSuggestions=html;this.resultsEl.empty().html(html);if(this.notAnsweredResults.length){var x;
for(x=0;x<this.notAnsweredResults.length;x++){$("li."+this.notAnsweredResults[x],this.resultsEl).remove()
}}if(!$("li:first",this.resultsEl).length){this.suggestionsBox.hide();this.lastSuggestions=null}else{if(!this.moreLink.data("has-mored")){this.moreLink.data("has-mored",true);
var count=$("li",this.resultsEl).length;if(count>6){var remainCount=count-6;$(".count",this.moreLink).text(remainCount);
this.moreLink.show();$("li",this.resultsEl).slice(5).hide()}else{this.moreLink.hide()}}else{this.moreLink.hide()
}var self=this;$("li a[href]",this.suggestionsBox).on("click",function(ev){ev.preventDefault();self.openSuggestedContent($(this))
});this.suggestionsBox.show()}}})},openSuggestedContent:function(aEl){var origUrl=aEl.attr("href");var url=Orb.appendQueryData(origUrl,"_partial","overlaySuggest");
var contentType=aEl.data("content-type");var contentId=aEl.data("content-id");var self=this;var tplEl=$(".related-content-overlay-tpl",this.el);
if(!tplEl[0]){window.open(origUrl);return}var overlay=new DeskPRO.User.SuggestedContentOverlay({template:tplEl.get(0).innerHTML,url:url,pageUrl:origUrl,contentType:aEl.data("content-type"),contentId:aEl.data("content-id"),destroyOnClose:true,openNear:$(".dp-related-search",this.el),onInit:(function(overlayEl,controls,overlay){$(".dp-set-answered",controls).on("click",function(ev){ev.preventDefault();
self.fireEvent("resolved",[contentType,contentId,true])});$(".dp-answererd",controls).on("click",function(ev){ev.preventDefault();
var type=$(this).data("type");if(type=="close"){self.fireEvent("resolvedRedirect",[origUrl,contentType,contentId,this])
}else{overlay.close()}});$(".dp-not-answered",controls).on("click",function(ev){ev.preventDefault();self.fireEvent("notResolved",[contentType,contentId,true]);
aEl.parent().addClass("not-answered");self.notAnsweredResults.push(contentType+"-"+contentId);overlay.close()
})}).bind(this)});overlay.open()}});Orb.createNamespace("DeskPRO.User.ElementHandler");DeskPRO.User.InlineLoginForm=new Orb.Class({Implements:[Orb.Util.Options,Orb.Util.Events],initialize:function(options){var self=this;
this.options={emailSel:"#dp_inline_login_email",passwordSel:"#dp_inline_login_pass",context:null};this.setOptions(options);
this.context=this.options.context||document;this._initLoginForm($(".dp-inline-login",this.context))},_initLoginForm:function(wrapper){var self=this;
this.el=wrapper;this.loginWrapper=wrapper;this.passwordRow=$(".dp-inline-login-pass",wrapper);this.nonloginWrapper=$(".dp-inline-non-login",this.context);
this.loginBtn=$(".dp-login-trigger",wrapper);this.loginSection=$(".dp-login-section",this.el);this.resetSection=$(".dp-reset-section",this.el);
$(".dp-inline-login-open",wrapper).on("click",(function(ev){ev.preventDefault();ev.stopPropagation();
if(this.loginWrapper.is(".open")){this.closeLogin()}else{this.openLogin()}}).bind(this));this.loginBtn.on("click",(function(ev){ev.preventDefault();
ev.stopPropagation();this.processLogin()}).bind(this));$(this.options.passwordSel,wrapper).on("keypress",function(ev){if(ev.keyCode==13){ev.preventDefault();
if(self.isOpen()){self.processLogin()}}});$(this.options.emailSel,wrapper).on("keypress",function(ev){if(ev.keyCode==13){ev.preventDefault();
if(self.isOpen()){self.processLogin()}}});this._initResetSection()},isOpen:function(){return this.loginWrapper.is(".open")
},openLogin:function(){this.loginWrapper.addClass("open");this.passwordRow.slideDown("fast");this.nonloginWrapper.animate({opacity:"0.4",duration:"fast"})
},closeLogin:function(){this.passwordRow.slideUp("fast",(function(){this.loginWrapper.removeClass("open")
}).bind(this));this.nonloginWrapper.animate({opacity:"1",duration:"fast"})},processLogin:function(){var postData=[];
postData.push({name:"email",value:$(this.options.emailSel,this.context).val()});postData.push({name:"password",value:$(this.options.passwordSel,this.context).val()});
if(this.loginBtn.is(".mode-advanced")){postData.push({name:"mode",value:"advanced"})}$.ajax({url:BASE_URL+"login/inline-login",type:"POST",data:postData,dataType:"json",context:this,success:function(data){var newEl=$(data.html);
if(data.person_id){DeskPRO_Window.initFeatures(newEl);$("#dp_inline_login_row").replaceWith(newEl);this.nonloginWrapper.css({opacity:"1"})
}else{$("#dp_inline_login_row").replaceWith(newEl);$(".dp-inline-login-pass",newEl).show()}if(data.sections_replace){Object.each(data.sections_replace,function(html,id){$("#"+id).empty().replaceWith(html)
})}this._initLoginForm(newEl);this.fireEvent("success",[data,this])}})},_initResetSection:function(){$(".forgot",this.el).on("click",(function(ev){ev.preventDefault();
this.showReset()}).bind(this));$(".back",this.resetSection).on("click",(function(ev){this.hideReset()
}).bind(this));$(".dp-do-send",this.resetSection).on("click",(function(ev){ev.preventDefault();this.sendReset()
}).bind(this))},sendReset:function(){this.resetSection.addClass("loading");$.ajax({url:BASE_URL+"login/reset-password/send",type:"POST",data:{email:$(this.options.emailSel,this.el).val()},dataType:"json",context:this,success:function(){this.resetSection.removeClass("loading");
var descEl=$(".dp-reset-desc",this.resetSection);var sentEl=$(".dp-reset-sent",this.resetSection);descEl.slideUp("fast",function(){sentEl.slideDown()
})}})},showReset:function(){this.loginSection.slideUp("fast",(function(){this.resetSection.slideDown("fast")
}).bind(this))},hideReset:function(quick){if(quick){this.resetSection.hide();this.loginSection.show();
$(".dp-reset-desc",this.resetSection).show();$(".dp-reset-sent",this.resetSection).hide()}else{this.resetSection.slideUp("fast",(function(){this.loginSection.slideDown("fast");
$(".dp-reset-desc",this.resetSection).show();$(".dp-reset-sent",this.resetSection).hide()}).bind(this))
}}});Orb.createNamespace("DeskPRO.Form");DeskPRO.Form.FormValidator=new Orb.Class({Implements:[Orb.Util.Events],initialize:function(el){var self=this;
this.el=$(el);var attachWaitMessage=this.el.find(".attach-is-loading-warn");var submitting=false;if(this.el.is("form")){this.el.on("submit",function(ev){if(submitting){ev.preventDefault();
return}$(".dp-error").removeClass("dp-error");self.validateAll();if(self.hasErrors()){var debugbox=$("#debug_box");
if(debugbox[0]){var append=["Fields with errors:"];$(".dp-error").each(function(){var field=$(this).find("input, textarea, select").first();
append.push(field.attr("name")||field.attr("id")||field.val())});debugbox.text(debugbox.text()+"\n"+append.join("\n"))
}ev.preventDefault()}else{var isUploading=false;self.el.find(".form-upload-section").each(function(){if($(this).find(".uploading")[0]){isUploading=true;
$(this).one("dp_upload_all_done",function(ev){attachWaitMessage.hide();self.el.submit()});return false
}});if(isUploading){ev.preventDefault();attachWaitMessage.show()}else{submitting=true}}});$("[required]",self.el).each(function(){$(this).attr("required",false)
})}this.refreshElements()},refreshElements:function(){var el=this.el;this.formElements=$("[data-field-validators]",this.el).each(function(){var names,i,validators,className,classObj;
names=$(this).data("field-validators").split(",");validators=[];for(i=0;i<names.length;i++){if($(this).data("field-validators-inst")){Array.each($(this).data("field-validators-inst"),function(v){v.destroy()
});$(this).data("field-validators-inst",null)}var className=names[i].trim();var classObj=Orb.getNamespacedObject(className);
if(!classObj){DP.console.error("Unknown form validator `%s` on element %o",className,this);continue}var obj=new classObj($(this));
validators.push(obj)}if(validators.length){$(this).data("field-validators-inst",validators)}})},validateAll:function(){this.el.find(".dp-error.dp-error-static").removeClass("dp-error");
this.formElements.each(function(){var validators,i;if(!$(this).is(":visible")){return}validators=$(this).data("field-validators-inst");
for(i=0;i<validators.length;i++){validators[i].validate("submit")}})},hasErrors:function(){if($(".dp-error",this.el).filter(":visible").length>0){return true
}return false}});Orb.createNamespace("DeskPRO.Form");DeskPRO.Form.FieldValidator=new Orb.Class({Implements:[Orb.Util.Events],initialize:function(el){this.el=$(el);
if(this.el.data("val-wrap-sel")){this.wrapper=this.el.closest(this.el.data("val-wrap-sel"))}else{this.wrapper=this.el.closest(".dp-form-row, .dp-control-group").first()
}this.init()},init:function(){},setErrorCodes:function(codes){console.log("Errors on %o: %o",this,codes);
this.wrapper.removeClass("dp-error-"+this._getMyErrorCodes().join(" dp-error-"));if(codes&&codes.length){if(typeof codes=="string"){codes=[codes]
}this.wrapper.addClass("dp-error-"+codes.join(" dp-error-"));this.wrapper.addClass("dp-error")}else{if(!this.wrapper.is('[class*="dp-error-"]')){this.wrapper.removeClass("dp-error")
}}},hasError:function(){if(this.wrapper.is(".dp-error-"+this._getMyErrorCodes().join(", .dp-error-"))){return true
}return false},validate:function(action){},_getMyErrorCodes:function(){},destroy:function(){}});Orb.createNamespace("DeskPRO.Form");
DeskPRO.Form.LengthValidator=new Orb.Class({Extends:DeskPRO.Form.FieldValidator,init:function(){var self=this;
this.min=1;this.max=-1;this.excludeBlank=true;if(this.el.data("min-len")){this.min=parseInt(this.el.data("min-len"))
}if(this.el.data("max-len")){this.max=parseInt(this.el.data("max-len"))}if(this.el.data("exclude-blank")){this.excludeBlank=parseInt(this.el.data("exclude-blank"));
this.excludeBlank=this.excludeBlank?true:false}this.el.on("change",function(){self.validate()})},validate:function(action){var len=0;
if(this.el.is("select")){if(this.excludeBlank){$("option:selected",this.el).each(function(){if($(this).val()!="0"&&$(this).val().trim()!==""){len++
}})}else{len=$("option:selected",this.el).length}}else{len=this.el.val().trim().length}var errorCodes=[];
if(this.min>-1&&len<this.min){errorCodes.push("len_too_short")}if(this.max>-1&&len>this.max){errorCodes.push("len_too_long")
}this.setErrorCodes(errorCodes)},_getMyErrorCodes:function(){return["len_too_long","len_too_short"]}});
Orb.createNamespace("DeskPRO.Form");DeskPRO.Form.EmailValidator=new Orb.Class({Extends:DeskPRO.Form.FieldValidator,init:function(){var self=this;
this.el.on("change",function(){self.validate("change")})},validate:function(action){var len=0;var val=this.el.val().trim();
if(val.length<3||val.indexOf("@")<1){this.setErrorCodes(["invalid_email"])}else{this.setErrorCodes([])
}},_getMyErrorCodes:function(){return["invalid_email"]}});Orb.createNamespace("DeskPRO.Form");DeskPRO.Form.RegexValidator=new Orb.Class({Extends:DeskPRO.Form.FieldValidator,init:function(){var self=this;
this.el.on("change",function(){self.validate("change")})},validate:function(action){var len=0;var val=this.el.val().trim();
if(!this.regex){if(this.el.data("regex-modifiers")){this.regex=new RegExp(this.el.data("regex"),this.el.data("regex-modifiers"))
}else{this.regex=new RegExp(this.el.data("regex"))}}if(!this.regex.test(val)){this.setErrorCodes(["pattern_no_match"])
}else{this.setErrorCodes([])}},_getMyErrorCodes:function(){return["pattern_no_match"]}});Orb.createNamespace("DeskPRO.Form");
DeskPRO.Form.TwoLevelSelectValidator=new Orb.Class({Extends:DeskPRO.Form.FieldValidator,init:function(){var self=this;
this.parentSel=$("select.parent-option",this.el);this.childSel=$(".dp-sub-options > select",this.el);
this.parentSel.on("change",function(){self.validate("change")});this.childSel.on("change",function(){self.validate("change")
})},validate:function(action){if(action=="change"&&!this.hasError()){return}var errorCodes=[];var visibleChild=this.childSel.filter(":visible");
if(visibleChild.length){var val=$("option:selected",visibleChild).val();if(val===""||val=="0"){errorCodes.push("select_child_value")
}}else{var val=$("option:selected",this.parentSel).val();if(val===""||val=="0"){errorCodes.push("select_value")
}}this.setErrorCodes(errorCodes)},_getMyErrorCodes:function(){return["select_value","select_child_value"]
}});